...
export type ParticipantSelector = (
  candidates: Actor[],
 ...
 */
export const randomSelector: ParticipantSelector = (
  candidates,
  criteria,
  now = Date.now()
) => {
 ...
 */
export const capabilitySelector: ParticipantSelector = (
  candidates,
  criteria,
  now = Date.now()
) => {
 ...
 */
export const createRoundRobinSelector = (): ParticipantSelector => {
  ...
  return (candidates, criteria, now = Date.now()) => {
    ...
    const selected: ParticipantCandidate[] = [];
 ...
 */
export const reputationSelector: ParticipantSelector = (
  candidates,
  criteria,
  now = Date.now()
) => {
  const eligible = candidates
    ...
    .map(actor => {
      ...
      const reputation = (actor as any).reputation ?? 0;
      return {
        actor_id: actor.id,
        actor,
...
export async function selectParticipants(
  ...
  criteria: SelectionCriteria,
  selector: ParticipantSelector = randomSelector,
  audit: AuditWriter,
  now: number = Date.now()
): Promise<{
  ...
  const eligibleCount = availableActors.filter(actor =>
    actor.id !== criteria.target_id &&
    !criteria.exclude_actors?.includes(actor.id) &&
    !isParticipantOnCooldown(actor.id, now) &&
    (criteria.required_capabilities?.every(cap => actor.capabilities.includes(cap)) ?? true)
 ...
 */
export const darpComplianceSelector: ParticipantSelector = (
  candidates,
  criteria,
  now = Date.now()
) => {
  ...
  const eligible = candidates
    ...
    .map(actor => {
      const complianceCaps = actor.capabilities.filter(cap =>
        complianceCapabilities.some(required => cap.includes(required))
      );
 ...
 */
export function getSelectionStrategy(name: string): ParticipantSelector {
  switch (name) {
