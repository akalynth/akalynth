...
} from '../receipt/hasher.js';
import {
  ABSENCE_CODE,
  type AbsenceReceiptInputs,
  type AbsenceOutcome,
  type AbsenceFinding,
  type AbsenceComputed,
  type AbsenceCode,
} from './types.js';
...
export interface VerifyAbsenceOpts {
  /** When provided, Ed25519 signatures over the verified slice are checked. */
 ...
 */
export function verifyAbsenceClaim(
  chain: CoordinationReceipt[],
  inputs: AbsenceReceiptInputs,
  opts: VerifyAbsenceOpts = {},
): AbsenceOutcome {
  const findings: AbsenceFinding[] = [];
  const computed: AbsenceComputed = {
    predicate_hash: '',
  ...
  const finish = (
    result: AbsenceOutcome['result'],
    extra: AbsenceFinding[],
    matched_count = 0,
  ): AbsenceOutcome => ({
    result,
  ...
  const invalid = (code: AbsenceCode, message: string, data?: Record<string, unknown>) =>
    finish('absence_invalid', [{ code, severity: 'error', message, data }]);
  const unprovable = (code: AbsenceCode, message: string, data?: Record<string, unknown>) =>
    finish('absence_unprovable', [{ code, severity: 'warn', message, data }]);

  const receipts = [...chain].sort((a, b) => a.sequence - b.sequence);
  const { from_seq, to_seq } = inputs.interval ?? ({} as AbsenceReceiptInputs['interval']);

  // ---- schema sanity ----
  if (
    !Number.isInteger(from_seq) ||
  ...
  const bySeq = new Map<number, CoordinationReceipt>();
  for (const r of receipts) bySeq.set(r.sequence, r);
  for (let s = 1; s <= to_seq; s++) {
    if (!bySeq.has(s)) {
      return unprovable(
        ABSENCE_CODE.LOG_GAP,
        `missing receipt at sequence ${s} (need contiguous 1..${to_seq})`,
      );
    }
  }
  ...
  for (let s = 1; s <= to_seq; s++) {
    ...
    if (!h.ok) return invalid(ABSENCE_CODE.CHAIN_INVALID, `hash mismatch at seq ${s}: ${h.reason}`);
  ...
  if (hasAuthorityTransition(receipts, from_seq, to_seq)) {
    return unprovable(
      ABSENCE_CODE.AUTHORITY_TRANSITION,
      `authority changed within [${from_seq}..${to_seq}]; segment into per-epoch sub-receipts`,
    );
  }
  ...
  if (!inputs.boundary.capture_completeness_ref) {
    findings.push({
      ...
      severity: 'info',
      message:
        'capture completeness not attested: claim is bounded to this log only, not "did it happen".',
    });
  }
  ...
  for (let s = from_seq; s <= to_seq; s++) {
    if (evaluatePredicate(inputs.predicate.definition, bySeq.get(s)!)) matchSeqs.push(s);
  }
  ...
  if (inputs.proof.matched_count !== 0) {
    return invalid(
      ABSENCE_CODE.PREDICATE_MISMATCH,
      `proof.matched_count claims ${inputs.proof.matched_count} but re-execution found 0`,
    );
  }
