...
import { computeDeathDrops } from './dropPolicy.js';
import { blake3Prefixed, canonicalJson } from './hashPrimitive.js';
import { verifyGlobalChainSlice, verifySignedChainSlice, } from './chronicleChain.js';
// ---------------------------------------------------------------------------
...
function computeReceiptHash(receipt) {
    const { event_hash: _eh, signature: _sig, ...contentFields } = receipt;
...
function verifyReceiptSignature(prevHash, eventHash, signatureHex, publicKeyHex) {
    try {
...
export function verifyOutcomeFromReceipt(receipt, context) {
    if (!isObject(receipt)) {
        return {
...
function verifyCombatResolved(receipt, context) {
    const reason_codes = [];
    // ----- Static checks that always run -----
    const receipt_authenticity = 'not_checked';
    reason_codes.push('RECEIPT_SIGNATURE_NOT_CHECKED');
    const chronicle_inclusion = 'not_checked';
    ...
    const precommit_anchoring = 'fail';
    ...
    const outcome_derivation = 'unsupported';
    ...
    if (!isObject(inputs)) {
        ...
        if (!('target_player_id' in inputs))
            shapeReasons.push('MISSING_TARGET_PLAYER_ID');
        if (!('map' in inputs))
            shapeReasons.push('MISSING_MAP');
        if (!('position' in inputs))
            shapeReasons.push('MISSING_POSITION');
        if (!('outcome' in inputs))
            shapeReasons.push('MISSING_OUTCOME');
        if (!('dropped_item_ids' in inputs))
            shapeReasons.push('MISSING_DROPPED_ITEM_IDS');
        if (!('drop_seed_hash' in inputs))
            shapeReasons.push('MISSING_DROP_SEED_HASH');
    }
    ...
    const checkedFailed = !seedBindingOk || (rng_commit_reveal === 'fail');
    let final_status;
    if (checkedFailed) {
        final_status = 'failed';
    }
...
function reconstructItemsForDrop(raw) {
...
function buildHeatLookup(items) {
    const m = new Map();
...
function verifyReceiptRngProof(receipt, inp, context) {
    const reason_codes = [];
    const receipt_authenticity = 'not_checked';
    reason_codes.push('RECEIPT_SIGNATURE_NOT_CHECKED');
    const chronicle_inclusion = 'not_checked';
    ...
    if (typeof bodyHash !== 'string' || bodyHash.length === 0) {
        ...
        if (!bodyHashOk)
            reason_codes.push('RECEIPT_BODY_HASH_MISMATCH');
    }
    // ----- rng_commit_reveal: commit binding + per-index RNG output -----
    let rng_commit_reveal = 'pass';
    ...
    if (!isObject(dInputs)) {
        ...
        if (hasItemsArray) {
            ...
            if (items === null ||
                ...
                const recomputed = computeDeathDrops(items, map, reputation, revealSeed, [], heatLookup).droppedItemIds;
                const expected = dropped;
                const sameLen = recomputed.length === expected.length;
                const sameItems = sameLen && recomputed.every((id, i) => id === expected[i]);
                if (!sameItems) {
                    reason_codes.push('OUTCOME_MISMATCH');
                    outcome_derivation = 'fail';
                }
            ...
            if (!opening) {
                ...
                if (recomputedCommit !== storedCommit) {
                    ...
                    if (typeof reputation !== 'number' ||
                        ...
                        const recomputed = computeDeathDrops(items, map, reputation, revealSeed, [], heatLookup).droppedItemIds;
                        const expected = dropped;
                        const same = recomputed.length === expected.length &&
    ...
    const hardFail = !bodyHashOk || rng_commit_reveal === 'fail' || outcome_derivation === 'fail';
    let final_status;
    if (hardFail) {
        final_status = 'failed';
    }
...
function findDeathEventIndex(slice, eventPreimageHash, victimDid) {
    let found = -1;
    for (let i = 0; i < slice.length; i++) {
        const e = slice[i];
        if (e.event_type !== 'death')
            continue;
        if (e.actor !== victimDid)
            continue;
        if (e.payload?.['drop_seed_hash'] !== eventPreimageHash)
            continue;
        if (found !== -1)
            return -2; // ambiguous: more than one match
        found = i;
    }
    return found;
}
...
function findRngEventIndex(slice, eventType, victimDid, field) {
    for (let i = 0; i < slice.length; i++) {
        const e = slice[i];
        ...
        if (e.actor !== victimDid)
            continue;
        if (e.payload?.['rng_domain'] !== V1_COMMIT_DOMAIN)
            continue;
        if (!isStringNonEmpty(e.payload?.[field]))
            continue;
        return i;
    }
    return -1;
}
function verifyReceiptRngProofV2(receipt, inp, proof, context) {
    const reason_codes = [];
    ...
    if (!isStringNonEmpty(eventPreimageHash)) {
        ...
        if (!preimageOk)
            reason_codes.push('EVENT_PREIMAGE_HASH_MISMATCH');
    }
    ...
    let chronicle_inclusion;
    let precommit_anchoring;
    ...
    let commitActorFromSlice = null;
    let sliceCommitValue = null;
    ...
    if (!haveSlice) {
        ...
        const chainRes = verifyGlobalChainSlice(slice);
        if (!chainRes.ok) {
            chainBroken = true;
            ...
            if (deathPos < 0) {
                ...
                sliceCommitValue =
                    entries[commitPos].payload?.['rng_commit'] ?? null;
                ...
                if (revealPos < 0) {
                    ...
                    reveal = entries[revealPos].payload?.['rng_reveal'] ?? null;
    ...
    if (chronicleHardFailed) {
        ...
        if (!isStringNonEmpty(commitActor)) {
            ...
            const expectCommit = rngCommitV1(isStringNonEmpty(eventDomain) ? eventDomain : V1_COMMIT_DOMAIN, commitActor, revealStr);
            const commitMatches = expectCommit === refCommit &&
        ...
        if (!isObject(dInputs) || !derivedSeed) {
            ...
            if (items === null ||
                ...
                const heatLookup = buildHeatLookup(items);
                const recomputed = computeDeathDrops(items, map, reputation, derivedSeed, [], heatLookup).droppedItemIds;
                const expected = dropped;
                const same = recomputed.length === expected.length &&
                    recomputed.every((id, i) => id === expected[i]);
                if (!same) {
    ...
    if (!preimageOk) {
        if (outcome_derivation === 'pass')
            outcome_derivation = 'fail';
        if (rng_commit_reveal === 'pass')
            rng_commit_reveal = 'fail';
    }
    ...
    const receiptVerifyKey = isStringNonEmpty(ctx.signingPublicKeyHex)
        ...
        : isStringNonEmpty(ctx.authPublicKeyHex)
            ...
            : null;
    let receipt_authenticity;
    if (receiptVerifyKey === null) {
        receipt_authenticity = 'not_checked';
        reason_codes.push('RECEIPT_SIGNATURE_NOT_CHECKED');
    }
    else if (!isStringNonEmpty(receipt['prev_hash']) ||
        ...
        const ok = verifyReceiptSignature(receipt['prev_hash'], receipt['event_hash'], receipt['signature'], receiptVerifyKey);
        receipt_authenticity = ok ? 'pass' : 'fail';
        if (!ok)
            reason_codes.push('RECEIPT_SIGNATURE_INVALID');
    ...
    const bindingAndDerivationOk = rng_commit_reveal === 'pass' && outcome_derivation === 'pass';
    ...
    // ----- final_status -----
    const hardFail = rng_commit_reveal === 'fail' ||
        outcome_derivation === 'fail' ||
        receipt_authenticity === 'fail' ||
        chronicle_inclusion === 'fail' ||
        chainBroken ||
    ...
    const allPass = precommit_anchoring === 'pass' &&
