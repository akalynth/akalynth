 ...
 */
export function createTimeoutManager(config, audit) {
    return {
 ...
 */
export function createAutoTimeoutProcessor(timeoutManager, intervalMs = 30000 // Default: 30 seconds
) {
 ...
 */
export function calculateRecommendedTtl(participantCount, baseResponseTimeMs = 60000 // 1 minute base
) {
 ...
 */
export function formatTimeRemaining(expiresAt, now = Date.now()) {
 ...
 */
export function isUrgent(request, urgentThresholdMs = 60000) {
    const now = Date.now();
