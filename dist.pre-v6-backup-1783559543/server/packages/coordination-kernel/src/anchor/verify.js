...
import { verifyMerklePath, } from './client.js';
/**
 ...
 */
export async function verifyAnchoredBundle(bundle, anchor, client, verifyExternal = false) {
 ...
 */
export async function verifyBatchAnchoredBundle(bundle, batch_merkle_root, merkle_path, anchor, client, verifyExternal = false) {
    // Step 1: Verify bundle integrity
    const integrityResult = verifyBundleIntegrity(bundle);
    if (!integrityResult.valid) {
        return {
    ...
    const path_valid = verifyMerklePath(bundle.integrity.content_hash, batch_merkle_root, merkle_path);
    if (!path_valid) {
        return {
            valid: false,
            bundle_integrity: true,
 ...
 */
export function verifyAnchoredBundleOffline(bundle, anchor, batch_merkle_root, merkle_path) {
    // Step 1: Verify bundle integrity
    const integrityResult = verifyBundleIntegrity(bundle);
    if (!integrityResult.valid) {
        return {
