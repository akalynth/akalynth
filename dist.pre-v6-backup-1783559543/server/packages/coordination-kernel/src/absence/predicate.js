 ...
 */
function resolveField(receipt, field) {
    const parts = field.split('.');
    const root = parts[0];
    if (!ALLOWED_ROOTS.has(root))
        return { found: false, value: undefined };
    if (root !== 'inputs' && parts.length > 1)
        return { found: false, value: undefined };
    let cur = root === 'action'
        ? receipt.action
        : root === 'actor_id'
...
export function evaluatePredicate(predicate, receipt) {
    switch (predicate.op) {
        case 'eq': {
...
function validField(field) {
    ...
    if (!ALLOWED_ROOTS.has(root))
        return { ok: false, reason: `field root not allowed: ${root}` };
 ...
 */
export function validatePredicate(predicate, depth = 0) {
    if (depth > MAX_DEPTH)
        return { ok: false, reason: 'predicate nesting too deep' };
    ...
    switch (p.op) {
        case 'eq':
            if (typeof p.field !== 'string')
                return { ok: false, reason: 'eq.field must be a string' };
            return validField(p.field);
        case 'in':
            if (typeof p.field !== 'string')
                return { ok: false, reason: 'in.field must be a string' };
            if (!Array.isArray(p.value))
                return { ok: false, reason: 'in.value must be an array' };
            return validField(p.field);
        case 'exists':
            if (typeof p.field !== 'string')
                return { ok: false, reason: 'exists.field must be a string' };
            return validField(p.field);
        case 'and':
        case 'or': {
