...
export const WITNESS_MOTH_BLOOM_CONTRIBUTIONS = [
...
export const WITNESS_MOTH_BLOOM_EVIDENCE = [
...
export function createWitnessMothBloomRuntime() {
    return {
...
export function parseWitnessMothBloomSkillId(skillId) {
    if (!skillId.startsWith(WITNESS_MOTH_BLOOM_SKILL_PREFIX))
        return null;
    if (skillId.startsWith(WITNESS_MOTH_BLOOM_EVIDENCE_PREFIX))
        return null;
...
export function parseWitnessMothBloomEvidenceSkillId(skillId) {
    if (!skillId.startsWith(WITNESS_MOTH_BLOOM_EVIDENCE_PREFIX))
        return null;
...
export function parseWitnessMothBloomSkillIntent(skillId) {
    const evidenceId = parseWitnessMothBloomEvidenceSkillId(skillId);
    if (evidenceId)
        return { kind: 'evidence', evidence_id: evidenceId };
    const contributionId = parseWitnessMothBloomSkillId(skillId);
    if (contributionId)
        return { kind: 'contribution', contribution_id: contributionId };
    return null;
}
export function witnessMothBloomPublicState(runtime) {
    const teaser = runtime.teaser?.unlocked
        ? { id: runtime.teaser.id, unlocked: true }
        : undefined;
    return {
...
export function hydrateWitnessMothBloomRuntime(runtime, row) {
    if (!row || row.event_id !== runtime.event_id || row.map !== runtime.map)
        return false;
    if (!isWitnessMothBloomPhase(row.phase))
        return false;
...
export function startWitnessMothBloom(runtime, input, writeReceipt) {
...
export function recoverWitnessMothBloomEvidence(runtime, input, writeReceipt) {
    if (input.map !== runtime.map) {
        return {
            ok: false,
            reason: 'invalid_target',
...
export function recordWitnessMothBloomContribution(runtime, input, writeReceipt) {
    if (input.map !== runtime.map) {
        return {
            ok: false,
            reason: 'invalid_target',
    ...
    const phaseForReceipt = 'investigation';
    writeReceipt({
        player_id: input.player_id,
    ...
    const outcome = 'controlled_release';
    writeReceipt({
        player_id: input.player_id,
...
export function handleWitnessMothBloomSkillIntent(runtime, input, writeReceipt) {
    ...
    if (intent.kind === 'evidence') {
        return recoverWitnessMothBloomEvidence(runtime, {
            player_id: input.player_id,
            map: input.map,
    ...
    return recordWitnessMothBloomContribution(runtime, {
        player_id: input.player_id,
        map: input.map,
...
function parseHydratedEvidence(raw) {
    let parsed;
    try {
    ...
    for (const [key, value] of Object.entries(parsed)) {
        if (!isWitnessMothBloomEvidenceId(key))
            continue;
        if (!value || typeof value !== 'object' || Array.isArray(value))
            continue;
        const rawEvidence = value;
        const playerId = rawEvidence.player_id;
        const recoveredAt = rawEvidence.recovered_at_ms;
        if (typeof playerId !== 'string' || typeof recoveredAt !== 'number')
            continue;
        evidence[key] = {
            evidence_id: key,
            player_id: playerId,
...
function parseHydratedContributions(raw) {
    let parsed;
    try {
        parsed = JSON.parse(raw);
    }
    ...
    const contributions = {};
    for (const [key, value] of Object.entries(parsed)) {
        if (!isWitnessMothBloomContributionId(key))
            continue;
        if (!value || typeof value !== 'object' || Array.isArray(value))
            continue;
        const rawContribution = value;
        const playerId = rawContribution.player_id;
        const recordedAt = rawContribution.recorded_at_ms;
        if (typeof playerId !== 'string' || typeof recordedAt !== 'number')
            continue;
        contributions[key] = {
            contribution_id: key,
            player_id: playerId,
...
function parseHydratedTeaser(raw) {
    let parsed;
    try {
        parsed = JSON.parse(raw ?? '{}');
    }
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
        return null;
    const teaser = parsed;
    if (teaser.id !== EMBER_ROAD_TEASER_ID || teaser.unlocked !== true)
        return null;
    const unlockedBy = teaser.unlocked_by;
    const unlockedAt = teaser.unlocked_at_ms;
    if (typeof unlockedBy !== 'string' || typeof unlockedAt !== 'number')
        return null;
    return {
        id: EMBER_ROAD_TEASER_ID,
        unlocked: true,
...
function messageForContribution(contributionId) {
...
function messageForEvidence(evidenceId) {
    switch (evidenceId) {
