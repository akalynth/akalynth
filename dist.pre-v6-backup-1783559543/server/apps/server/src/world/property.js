...
import { PROPERTY_CREATED_ACTION, PROPERTY_LISTED_ACTION, PROPERTY_UNLISTED_ACTION, PROPERTY_PURCHASED_ACTION, PROPERTY_TRANSFERRED_ACTION, PROPERTY_AUCTION_OPENED_ACTION, PROPERTY_BID_ACTION, PROPERTY_BID_REFUNDED_ACTION, PROPERTY_AUCTION_SETTLED_ACTION, PROPERTY_AUCTION_CANCELLED_ACTION, MAX_GOLD_AMOUNT, } from '../../../../packages/shared/types.js';
// ============================================================================
// In-Memory Projection (receipt-derived)
// ============================================================================
const propertyById = new Map();
// One auction projection per property (open/settled/cancelled). Receipt-derived.
const auctionByPropertyId = new Map();
...
export function makePropertyId(zone, plotId) {
    return `${zone}:${plotId}`;
}
...
export function getPropertiesForOwner(playerId) {
...
export function isOwnedBy(propertyId, playerId) {
    return propertyById.get(propertyId)?.owner_player_id === playerId;
}
export function getAuction(propertyId) {
...
export function clearPropertyProjection() {
    propertyById.clear();
 ...
 */
export function hydrateProperty(p) {
 ...
 */
export function hydrateAuction(a) {
 ...
 */
export function ensurePropertiesSeeded(housePlots, zone, writeReceipt) {
    for (const plot of housePlots) {
        const propertyId = makePropertyId(zone, plot.id);
        if (propertyById.has(propertyId) || seededProperties.has(propertyId))
            continue;
 ...
 */
export function applyReceiptToProperty(receipt) {
    ...
    switch (receipt.action) {
        case PROPERTY_CREATED_ACTION: {
            ...
            if (propertyById.has(propertyId))
                break;
            const zone = receipt.inputs?.zone;
            const plotId = receipt.inputs?.plot_id;
            if (!zone || !plotId)
                break;
            ...
            propertyById.set(propertyId, {
                ...
                district: receipt.inputs?.district ?? null,
        ...
        case PROPERTY_TRANSFERRED_ACTION: {
            ...
            const propertyId = receipt.inputs?.property_id;
            const sellerId = receipt.inputs?.seller_id;
            const price = receipt.inputs?.price;
            if (!propertyId || !actorId || !sellerId)
                break;
            const prop = propertyById.get(propertyId);
            if (!prop)
                break;
        ...
        case PROPERTY_AUCTION_OPENED_ACTION: {
            ...
            const kind = receipt.inputs?.kind;
            if (kind !== 'primary' && kind !== 'resale')
                break;
            if (kind === 'primary') {
                if (prop.status !== 'unowned' || prop.owner_player_id !== null) {
            ...
            if (!isValidPrice(minBid) || !isValidPrice(minIncrement))
                break;
            ...
            auctionByPropertyId.set(propertyId, {
                ...
                scheduled_close_ms: typeof receipt.inputs?.scheduled_close_ms === 'number'
                    ? receipt.inputs.scheduled_close_ms
                    : null,
        ...
        case PROPERTY_AUCTION_SETTLED_ACTION: {
            ...
            const winnerId = receipt.inputs?.winner_id ?? null;
            if (winnerId) {
                const price = receipt.inputs?.price;
                const sellerId = receipt.inputs?.seller_id ?? auction.seller_id;
