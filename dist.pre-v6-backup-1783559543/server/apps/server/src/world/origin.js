 ...
 */
export function isOriginWorthy(action) {
 ...
 */
export function maybeSealOriginFromReceipt(audit, persist, receipt, receiptHash) {
    // Only origin-worthy actions can trigger sealing
    if (!isOriginWorthy(receipt.action))
        return;
    // Check if player already has an origin (idempotency check)
    if (hasOriginActSealed(persist.db, receipt.actor_id))
        return;
