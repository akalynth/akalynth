...
import { DROP_POLICY, getLegendaryHeat, } from '../../../../packages/shared/dropPolicy.js';
// Re-export the PURE single-sourced policy surface (byte-identical to shared).
export { DROP_POLICY, LEGENDARY_HEAT_DECAY_PER_MINUTE, computeDeathDrops, computeDropCount, selectItemsToDrop, getDeathDropDecayMs, getLegendaryHeat, setLegendaryHeat, addLegendaryHeat, decayLegendaryHeat, decayHeatForCarriedItems, } from '../../../../packages/shared/dropPolicy.js';
// ============================================================================
 ...
 */
export function explainDeathDrops(items, map, reputation, deathReceiptHash, heatLookup) {
...
function getItemWeightWithLookup(item, heatLookup) {
    const baseWeight = ITEM_BASE_WEIGHT[item.item_type] ?? ITEM_BASE_WEIGHT.unknown;
...
function computeLegendaryMultiplierExposed(tier, heat) {
