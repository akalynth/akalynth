...
import { PRESENCE_ENTERED_ACTION, PRESENCE_LINGERED_ACTION, PRESENCE_OBSERVED_ACTION, PRESENCE_LINGER_THRESHOLD_MS, PRESENCE_OBSERVE_THRESHOLD_MS, } from '../../../../packages/shared/types.js';
// Static place definitions (server config, not player-defined)
// For v0: whole maps are places, plus specific landmarks
const PLACE_BOUNDARIES = [];
/**
 * Register place boundaries from map data.
 * Called once per map during server startup.
 */
export function registerMapPlaces(map, mapName) {
 ...
 */
export function getPlaceAt(map, x, y) {
    // Check specific landmarks first (more specific), then whole map (less specific)
    const specificPlaces = PLACE_BOUNDARIES.filter((p) => p.map === map &&
        p.place_id.includes(':') &&
        x >= p.x &&
        x < p.x + p.width &&
        y >= p.y &&
    ...
    const mapPlace = PLACE_BOUNDARIES.find((p) => p.map === map && !p.place_id.includes(':'));
...
const coPresenceStart = new Map();
// Already observed pairs this session (to avoid spam)
const observedPairs = new Set();
function makeCoPresenceKey(placeId, playerA, playerB) {
 ...
 */
export function getPlayersInPlace(placeId) {
    const players = [];
 ...
 */
export function clearPresenceProjection() {
 ...
 */
export function onPlayerMoved(playerId, map, x, y, nowMs, writeReceipt) {
 ...
 */
export function onPresenceTick(playerId, nowMs, writeReceipt) {
    const state = getPresence(playerId);
    if (!state.current_place || !state.entered_at_ms)
        return;
 ...
 */
function checkCoPresence(playerA, playerB, placeId, nowMs, writeReceipt) {
 ...
 */
function clearCoPresenceForPlayer(playerId, placeId) {
    const keysToRemove = [];
 ...
 */
export function resetSessionState(playerId) {
    const state = getPresence(playerId);
 ...
 */
export function applyReceiptToPresence(receipt) {
    ...
    switch (receipt.action) {
        case PRESENCE_ENTERED_ACTION: {
            const placeId = receipt.inputs?.place_id;
            if (placeId) {
        ...
        case PRESENCE_OBSERVED_ACTION: {
            const placeId = receipt.inputs?.place_id;
            const otherId = receipt.inputs?.other_player_id;
            if (placeId && otherId) {
