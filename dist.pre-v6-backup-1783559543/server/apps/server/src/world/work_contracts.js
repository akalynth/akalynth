...
import { WORK_CONTRACT_STARTED_ACTION, WORK_CONTRACT_TICK_RECORDED_ACTION, WORK_CONTRACT_COMPLETED_ACTION, WORK_CONTRACT_FAILED_ACTION, WORK_CONTRACT_SCHEDULE, WORK_CONTRACT_TYPES, WALLET_CREDIT_ACTION, } from '../../../../packages/shared/types.js';
// ============================================================================
// In-Memory Projection (receipt-derived)
// ============================================================================
// Active contracts (in-memory, cleared on completion/failure)
const activeContractByPlayer = new Map();
// Cooldown tracking: when player can next start a contract
const cooldownByPlayer = new Map();
/**
 * Get active contract for a player (null if none).
 */
export function getActiveContract(playerId) {
    return activeContractByPlayer.get(playerId) ?? null;
}
/**
 * Check if player is on cooldown.
 */
export function isOnCooldown(playerId, nowMs) {
 ...
 */
export function getCooldownRemaining(playerId, nowMs) {
    const nextEligible = cooldownByPlayer.get(playerId) ?? 0;
 ...
 */
export function clearWorkContractsProjection() {
 ...
 */
export function startContract(playerId, contractType, nowMs, writeReceipt, contractIdOverride) {
 ...
 */
export function recordTick(playerId, contractId, nowMs, writeReceipt) {
 ...
 */
export function completeContract(playerId, contractId, nowMs, writeReceipt) {
    const contract = activeContractByPlayer.get(playerId);
    if (!contract || contract.contract_id !== contractId) {
        return { ok: false, error: 'invalid_contract' };
    }
 ...
 */
export function failContract(playerId, reason, writeReceipt) {
    const contract = activeContractByPlayer.get(playerId);
    if (!contract) {
        return false;
    }
 ...
 */
export function applyReceiptToWorkContracts(receipt) {
    const playerId = receipt.actor_id;
    if (!playerId)
        return;
    switch (receipt.action) {
        case WORK_CONTRACT_STARTED_ACTION: {
            const contractType = receipt.inputs?.contract_type;
            const contractId = receipt.inputs?.contract_id;
            const cooldownUntilIso = receipt.inputs?.cooldown_until;
            const startedAtMsRaw = receipt.inputs?.started_at_ms;
        ...
        case WORK_CONTRACT_TICK_RECORDED_ACTION: {
            const contractId = receipt.inputs?.contract_id;
            const tickAtMsRaw = receipt.inputs?.tick_at_ms;
            if (!contractId)
                break;
            const contract = activeContractByPlayer.get(playerId);
            if (!contract || contract.contract_id !== contractId)
                break;
            const tickAtMs = typeof tickAtMsRaw === 'number' && Number.isFinite(tickAtMsRaw)
                ? tickAtMsRaw
                : Date.parse(receipt.timestamp);
            if (Number.isNaN(tickAtMs))
                break;
