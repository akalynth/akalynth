import { SOVEREIGN_VOCATIONS, TEM_CHALLENGE_RESPONSE, VOCATION_DECLARED_ACTION, } from '../../../../packages/shared/types.js';
function defaultTutorialProgress() {
    return { move: false, chat: false, tem: false, gate: false, complete: false };
}
function defaultRookguardQuestInput() {
    return {
...
const rookguardQuestByPlayerId = new Map();
function cloneRookguardQuestInput(input) {
    return {
...
export function getRookguardQuestInput(playerId) {
    return cloneRookguardQuestInput(rookguardQuestByPlayerId.get(playerId) ?? defaultRookguardQuestInput());
}
export function clearRookguardQuestProjection() {
    rookguardQuestByPlayerId.clear();
}
function setRookguardQuestInput(playerId, next) {
    rookguardQuestByPlayerId.set(playerId, cloneRookguardQuestInput(next));
}
function validTutorialStep(value) {
    return value === 'move' || value === 'chat' || value === 'tem' || value === 'gate';
}
function validVocation(value) {
    return typeof value === 'string' && SOVEREIGN_VOCATIONS.includes(value);
}
export function applyReceiptToRookguardQuest(receipt) {
    ...
    if (next)
        setRookguardQuestInput(playerId, next);
}
export const ROOKGUARD_CODEX_SHELVES = [
    {
...
export const ROOKGUARD_CODEX_PROFESSIONS = {
    warden: {
        vocation: 'warden',
...
export function rookguardGateOpen(input) {
    return (input.tutorial.move &&
        input.tutorial.chat &&
        input.tutorial.tem &&
        input.trainingComplete &&
        input.vocation !== null);
}
export function rookguardQuestObjective(input) {
    if (!input.tutorial.move)
        return 'Step onto the glowing move rune (east plaza, tile 3,2)';
    if (!input.tutorial.chat)
        return 'Open Chat and send any message in Rookguard';
    if (!input.tutorial.tem)
        return `Walk to the Tem rune (tile 7,2) and answer ${TEM_CHALLENGE_RESPONSE}`;
    if (!input.trainingComplete)
        return 'Walk southeast to the training slime (tile 14,14) and tap Attack';
    if (!input.vocation)
        return 'Enter the guild hall and choose a Codex vocation';
    if (!input.tutorial.gate)
        return 'Walk onto the golden High City gate when it opens';
    return 'Rookguard Codex path complete';
}
export function buildRookguardQuestProgress(input) {
    const steps = [
        {
            step_id: 'move',
            label: 'Move rune',
...
export function buildOnwardRouteProgress(input, receiptProgress = {
