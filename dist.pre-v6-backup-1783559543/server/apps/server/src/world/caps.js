...
import { CAPABILITY_GRANTED_ACTION, CAPABILITY_REVOKED_ACTION, CAPABILITY_GATED_ACTION, CAP_HOUSE_BUY, CAP_ECHO_SPAWN, } from '../../../../packages/shared/types.js';
// Badge-to-capability derivation map (only active when CAPS_DEBUG_GRANT_SOVEREIGN=1)
const BADGE_DERIVED_CAPS = {
    sovereign: [CAP_HOUSE_BUY, CAP_ECHO_SPAWN],
};
/**
 * Check if a player has a specific capability.
 */
export function hasCap(player, cap) {
 ...
 */
export function grantCap(player, cap, source, audit, badge) {
    if (!player.caps)
        player.caps = [];
    if (player.caps.includes(cap))
 ...
 */
export function revokeCap(player, cap, source, audit) {
    if (!player.caps)
 ...
 */
export function applyBadgeDerivedCaps(player, audit) {
    const grantedCaps = [];
    if (!player.badges)
        return { grantedCaps };
 ...
 */
export function checkCapGate(player, requiredCap, attemptedAction, audit) {
    if (hasCap(player, requiredCap))
        return { allowed: true };
    audit.write({
        actor_id: player.id,
