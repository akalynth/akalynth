...
export function corsOriginAllowed(origin, policy) {
    const norm = normalizeOrigin(origin);
    if (!norm)
        return false;
    if (policy.allow.has(norm))
        return true;
    if (policy.allowLocalDev && isLocalDevOrigin(origin))
        return true;
    return false;
}
/**
 ...
 */
export function corsHeadersFor(origin, policy) {
    if (typeof origin !== 'string' || !corsOriginAllowed(origin, policy))
        return null;
    return {
