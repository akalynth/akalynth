...
function isGuestSessionError(x) {
    return typeof x.error === 'string';
}
function isSessionMeError(x) {
    return typeof x.error === 'string';
}
function isWorldPlayersError(x) {
    return typeof x.error === 'string';
}
function isWorldStateError(x) {
    return typeof x.error === 'string';
}
function isPublicReceiptsRawError(x) {
    return typeof x.error === 'string';
}
function isAntiCheatPriorError(x) {
    return typeof x.error === 'string';
}
export function handleHttp(req, res, deps) {
    ...
    if (method === 'GET' && path === '/v1/receipts/public') {
        ...
        if (offsetStr) {
            const offset = parseInt(offsetStr, 10);
            if (!isNaN(offset) && offset >= 0)
                params.offset = Math.min(offset, 10_000);
        }
    ...
    if (method === 'GET' && path === '/v1/receipts/public_raw') {
        if (!deps.queryPublicReceiptsRaw)
