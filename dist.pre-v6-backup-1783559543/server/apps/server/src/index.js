...
import { computeCapsHash, computeEventHash, computeGlobalEventHash, computePayloadHash, stripPayloadHashFields, } from '../../../packages/shared/chronicleChain.js';
import { canonicalJson } from '../../../packages/shared/hashPrimitive.js';
import { loadBuildInfo } from './build-info.js';
import { PROPERTY_LISTED_ACTION, PROPERTY_UNLISTED_ACTION, PROPERTY_PURCHASED_ACTION, PROPERTY_TRANSFERRED_ACTION, PROPERTY_AUCTION_OPENED_ACTION, PROPERTY_BID_ACTION, PROPERTY_BID_REFUNDED_ACTION, PROPERTY_AUCTION_CANCELLED_ACTION, FIRST_ATTEMPT_STONE_ACTION, HEAT_CHANGED_ACTION, LEDGER_MARKED_ACTION, HEAT_PENALTY_APPLIED_ACTION, HEAT_TEM_ESCALATION_ACTION, LEDGER_HESITATION_ACTION, LEGEND_ATTEMPTED_ACTION, LEGEND_REFUSED_ACTION, LEGEND_SIGHTED_ACTION, RUMOR_SEEDED_ACTION, } from '../../../packages/shared/types.js';
import { TileCode } from '../../../packages/shared/types.js';
import { DEATH_TEST_ENABLED, DEATH_RESPAWN_DELAY_MS, HEAT_DECAY_PER_MIN, HEAT_PENALTY_DURATION_MS, HEAT_PENALTY_THRESHOLD, HEAT_TEM_COOLDOWN_MS, HEAT_TEM_THRESHOLD, TICK_MS, } from '../../../packages/shared/constants.js';
import { handleHttp } from './api/http.js';
import { parseCorsOrigins, corsHeadersFor } from './api/cors.js';
import { createEmailSender, buildAccountEmail } from './account/email.js';
import { createAuditLogger } from './audit/logger.js';
import { LifecycleVerifierError, verifyLifecycleReceiptFile, } from './audit/lifecycleVerifier.js';
import { createReceiptsReader } from './audit/reader.js';
import path from 'node:path';
import { resolveChainPaths, logResolvedPaths, isProductionMode, validateKeyFile, } from '../../../packages/shared/paths.js';
...
import { makeWebEconomyRouter } from './economy/router.js';
import { publicActorForReceipt, toPublicReceipt } from './audit/public_receipts.js';
import { loadAuthKeyPair, signToken, verifyToken, generateNonce, getAuthKeyDomain, } from '../../../packages/coordination-kernel/src/identity/index.js';
...
import { createGatherSystem, startGather, startRefine, deliver, tickGather, onPlayerLeave, getPlayerGather, gatherProgressPct, refineProgressPct, isGatherEnabled, isRefineEnabled, DEFAULT_GATHER_CONFIG, AZURA_GATHER_NODES, AZURA_STATIONS, AZURA_REFINE_STATIONS, DELIVERY_RECORDED_ACTION, } from './world/gather.js';
import { indexFor, tryMove } from './world/movement.js';
import { registerMapPlaces, onPlayerMoved, onPresenceTick, onPlayerDisconnect, resetSessionState, getCurrentPlace, } from './world/presence.js';
...
import { handleAttackIntent, COMBAT_COOLDOWN_MS } from './world/combat.js';
import { getLegendaryHeat, setLegendaryHeat } from './world/drop-policy.js';
import { rngCommitV1, rngRevealHex32 } from './world/rng.js';
import { addHeat, createHeatState, hydrateHeatState, isPenaltyActive, shouldApplyPenalty, shouldTemEscalate, startPenalty, } from './world/heat.js';
import { findRunestoneTable, isNearRunestoneTable, rollRunestoneFace, runestoneWhisper, checkTrinityOfShadow, RUNESTONE_COOLDOWN_MS, RUNESTONE_BROADCAST_RADIUS, } from './world/runestone.js';
import { createWitnessRequest, selectWitnesses, getWitnessRequest, isWitnessRequestExpired, isWitnessInRequest, hasWitnessResponded, getWitnessPromptText, cleanupExpiredRequests, isTargetOnCooldown, recordWitnessResponse, tryResolveQuorum, getUnresolvedExpiredRequests, } from './world/witness.js';
import { handleUseSkill } from './skills/index.js';
import { handleGetModReports, handleModResolve } from './moderation/index.js';
import { RUNESTONE_CAST_ACTION, RUNESTONE_RESULT_ACTION, RUNESTONE_DENIED_ACTION, TRINITY_OF_SHADOW_ACTION, WITNESS_REQUESTED_ACTION, WITNESS_RESPONSE_ACTION, WITNESS_QUORUM_RESOLVED_ACTION, SOVEREIGN_DECLARED_ACTION, SOVEREIGN_PRESENCE_ACTION, SOVEREIGN_MARKED_ACTION, } from '../../../packages/shared/types.js';
import { applyBadgeDerivedCaps, hasCap } from './world/caps.js';
import { spawnEcho, despawnEcho, getEchoForMap, hasActiveEcho, echoToPublicPlayer, } from './world/echo.js';
import { initMobs, getMobsForMap, getMobById, hitMob, mobToPublicPlayer, tickMobRespawns, spawnMobLoot, } from './world/mobs.js';
import { CAP_ECHO_SPAWN } from '../../../packages/shared/types.js';
import { getEvidence } from './evidence/handler.js';
...
import { ensurePropertiesSeeded, hydrateProperty, hydrateAuction, getProperty, getAllProperties, getMarketListings, getAuction, minNextBid, isValidPrice, makePropertyId, } from './world/property.js';
...
import { startContract, recordTick, completeContract, failContract, getActiveContract, } from './world/work_contracts.js';
import { WITNESS_MOTH_BLOOM_EVENT_ID, createWitnessMothBloomRuntime, handleWitnessMothBloomSkillIntent, hydrateWitnessMothBloomRuntime, startWitnessMothBloom, witnessMothBloomPublicState, } from './world/world-events.js';
import { ROOKGUARD_CODEX_PROFESSIONS, buildOnwardRouteProgress, buildRookguardQuestProgress, getRookguardQuestInput, rookguardGateOpen, rookguardQuestObjective, } from './world/rookguardQuest.js';
...
import { VOCATION_DECLARED_ACTION, SOVEREIGN_PREFIX_GRANTED_ACTION, SOVEREIGN_PREFIX_REVOKED_ACTION, SOVEREIGN_VOCATIONS, VOCATION_LABEL, VOCATION_COSMETICS, WALLET_CREDIT_ACTION, WALLET_DEBIT_ACTION, MAX_GOLD_AMOUNT, } from '../../../packages/shared/types.js';
...
const EMAIL_TRANSPORT = process.env.EMAIL_TRANSPORT === 'smtp' ? 'smtp' : 'console';
...
const guildContributions = new Map();
...
const playersInsideHouse = new Map();
...
function parseEnvMs(envValue, fallback, min) {
    if (!envValue)
        return fallback;
...
function parseEnvInt(envValue, fallback, min) {
    if (!envValue)
        return fallback;
    const parsed = parseInt(envValue, 10);
    if (Number.isFinite(parsed) && parsed >= min)
...
function parseBoolEnv(envValue, fallback) {
...
function parseEnvIntClamped(envValue, fallback, min, max) {
    const parsed = parseEnvInt(envValue, fallback, min);
    return Math.min(parsed, max);
}
function parsePublicReceiptsActorMode(envValue) {
    return envValue === 'daily_hash' ? 'daily_hash' : 'anon';
}
function parsePublicReceiptsDelayProfile(envValue) {
...
const GUEST_SESSION_CLEANUP_MS = parseEnvMs(process.env.GUEST_SESSION_CLEANUP_MS, DEFAULT_GUEST_SESSION_CLEANUP_MS, 100);
...
function rejectInsecureHttp(res) {
    res.statusCode = 403;
    res.setHeader('content-type', 'application/json; charset=utf-8');
    res.setHeader('cache-control', 'no-store');
    res.end(JSON.stringify({ error: 'tls_required' }));
}
function rejectInsecureUpgrade(socket) {
    try {
...
function redactedActorForPlayerId(playerId, timestamp) {
    return publicActorForReceipt({
        ...
        inputs_hash: '',
        outputs_hash: '',
    }, PUBLIC_RECEIPTS_ACTOR_MODE, PUBLIC_RECEIPTS_HASH_SALT);
}
/**
 ...
 */
function verifyLifecycle(phase, fromSequence) {
 ...
 */
function receiptsPathHash(receiptsPath) {
 ...
 */
function checkIpConnectionLimit(ip, now) {
    if (!IP_RATE_LIMIT_ENABLED)
        return { allowed: true };
    if (!ip)
        return { allowed: true }; // Allow if IP unknown (shouldn't happen)
 ...
 */
function checkIpActionLimit(ip, action, now) {
    if (!IP_RATE_LIMIT_ENABLED)
        return { allowed: true };
    if (!ip)
        return { allowed: true };
...
function maybeRequestWitnesses(targetPlayerId, target, triggerKind, map, now) {
    if (!WITNESS_CONFIG.enabled)
        return;
    if (isTargetOnCooldown(targetPlayerId, now))
        return;
    const candidates = selectWitnesses(target, map, Array.from(sessions.values()).map((s) => ({
        connId: s.connId,
        player: s.player,
        inWorld: s.inWorld,
    ...
    const request = createWitnessRequest(requestId, targetPlayerId, targetActorRedacted, triggerKind, map, witnessIds, WITNESS_CONFIG, now);
    ...
    for (const candidate of candidates) {
        ...
        if (!session || !session.player)
            continue;
        send(session.ws, ServerMessages.temWitnessRequest(requestId, timestamp, map, targetActorRedacted, promptText, 'heat_penalty'));
    }
}
function emitQuorumResolutionReceipt(request, resolution) {
    audit.write({
...
const lastEventHashByActor = new Map();
...
// ============================================================================
function rebuildChronicleHeadsFromLog(logPath) {
...
function chronicleEvent(event_type, actorDid, caps, payload, rng = null) {
    ...
    const payloadCanonical = JSON.parse(canonicalJson(semanticPayload));
...
const protectedByPlayerId = new Map(); // player_id -> item_id
...
const accountService = new AccountService({
    ...
    hashPassword,
    verifyPassword,
    emitReceipt: (e) => audit.write({ player_id: e.accountId ?? 'system', action: e.action, inputs: e.inputs ?? {}, result: e.result }),
    now: () => Date.now(),
    config: {
...
const legendFirsts = new Set();
const legendSightedByPlayer = new Set();
const legendAttemptCountByPlayer = new Map();
// Sovereign presence tracking
let activeSovereignSessionId = null;
let activeSovereignPlayerId = null;
const PUBLIC_RECEIPTS_ALLOW = new Set([
    'death_in_rookguard',
    'death_in_azura',
...
const trinityEmitted = new Set();
const PUBLIC_RUMORS_ALLOW = new Set([RUMOR_SEEDED_ACTION]);
const ledgerHesitationByPlayer = new Map();
...
const RUMOR_NOTHING_FINISHES_MAP = 'Rookguard';
let rumorSeeded = false;
const LEGEND_STONE_ID = 'stone_cannot_obtain';
const LEGEND_STONE_MAP = 'Rookguard';
...
const RUMOR_STONE_REFUSES_MAP = 'Rookguard';
let stoneRumorSeeded = false;
const PUBLIC_RECEIPTS_ACTION_DELAYS_TIBIA = {
...
function publicReceiptDelayForAction(action) {
...
function ledgerHesitationDelayMs(playerId, deathTs) {
...
function summarizeLostItems(itemIds, persist) {
    if (itemIds.length === 0)
        return [];
...
function recordLedgerDeath(playerId, map, deathTs) {
    if (map === 'Rookguard')
        return;
    ledgerHesitationByPlayer.set(playerId, { death_ts: deathTs, map, applied: false });
}
function armLedgerHesitationIfNeeded(s) {
    if (!s.player)
        return;
...
function applyLedgerHesitationIfArmed(s) {
    if (!s.player)
        return null;
    if (!s.ledgerHesitationArmed || !s.ledgerHesitationDeathTs)
        return null;
    const state = ledgerHesitationByPlayer.get(s.player.id);
...
function seedRumorIfNeeded(playerId) {
    if (rumorSeeded)
        return;
    rumorSeeded = true;
    audit.write({
...
function seedStoneRumorIfNeeded(playerId) {
    if (stoneRumorSeeded)
        return;
...
function applyHeatChange(s, now, delta, reason, extra) {
    if (!s.player || delta === 0)
        return;
    ...
    if (extra?.window_ms)
        inputs.window_ms = extra.window_ms;
    audit.write({
...
function maybeEscalateHeat(s, now, reason) {
    if (!s.player)
        return;
...
function landmarkContains(pos, landmark) {
    return (pos.x >= landmark.x &&
        pos.y >= landmark.y &&
        pos.x < landmark.x + landmark.width &&
        pos.y < landmark.y + landmark.height);
}
function legendStoneLandmark(map) {
    const landmarks = map.landmarks;
    if (!landmarks || typeof landmarks !== 'object')
        return null;
    const stone = landmarks[LEGEND_STONE_LANDMARK];
    return asLandmarkBox(stone);
}
function toPublicRumor(receipt) {
...
const guestSessions = new Map(); // key = guest_token
const worlds = {
...
const gatherSystem = isGatherEnabled()
    ? createGatherSystem(DEFAULT_GATHER_CONFIG, AZURA_GATHER_NODES, isRefineEnabled() ? [...AZURA_STATIONS, ...AZURA_REFINE_STATIONS] : AZURA_STATIONS)
    : null;
...
const ZONE_DECAY_MINUTES = {
    Rookguard: null,
    Azura: 20,
};
function getZoneDecayMinutes(zone) {
...
function loadInventories() {
    ...
    for (const row of storageRows) {
        if (!houseStorage.has(row.property_id))
            houseStorage.set(row.property_id, new Set());
...
function sendHouseInventory(s, propertyId) {
    if (!s.player)
        return;
    const toInfo = (id) => toItemInfoFromPersist(id, persist.getItem.bind(persist));
...
function loadPropertiesAndSeed() {
    ...
    console.log(`[property] Loaded ${rows.length} properties, ${auctionRows.length} open auctions; ensured ${plots.length} Azura plots seeded`);
}
...
function ownerHistoryToPublic(p) {
 ...
 */
function mintStarterKit(playerId) {
    if (playerHasStarterKit(playerId))
        return;
...
function playerHasStarterKit(playerId) {
    for (const itemId of getPlayerInventoryIds(playerId)) {
        ...
        try {
            const meta = JSON.parse(item.meta_json);
            if (meta.source === STARTER_KIT_SOURCE)
                return true;
        }
        catch {
 ...
 */
function mintLegendaryItem(playerId, itemType = 'mark_token', tier = 1) {
    const meta = {
        legendary: true,
        legendary_tier: tier,
    };
...
function mintItemToInventory(playerId, itemType, meta, reason, source) {
    const writtenReceipt = audit.write({
        action: 'item_minted',
        player_id: playerId,
        inputs: {
 ...
 */
function handleDropItem(playerId, itemId, zone, x, y) {
    // Validate: player has item
    if (!inventory.get(playerId)?.has(itemId))
        return false;
 ...
 */
function handlePickupItem(playerId, itemId, zone, playerX, playerY) {
    ...
    for (const [zone, items] of worldItems) {
        for (const [itemId, item] of items) {
            if (item.decayAt && new Date(item.decayAt) <= now) {
                ...
                items.delete(itemId);
                broadcastToMap(zone, ServerMessages.worldItemRemoved(itemId));
            }
        }
    }
}
 ...
 */
function getPlayerInventoryIds(playerId) {
    return Array.from(inventory.get(playerId) ?? []);
}
function pruneExpiredGuestSessions(now) {
...
function resolveSessionMe(guest_token, expiredReason) {
    const now = Date.now();
...
function createCharacterHandler(name) {
    if (!authKeyPair) {
...
// determinism; no PII.
function issuePlayTokenForPlayer(playerId) {
    if (!authKeyPair)
        return null;
    const now = Date.now();
    const nonce = generateNonce();
...
const httpServer = http.createServer((req, res) => {
    ...
    const handled = handleHttp(req, res, {
        ...
        listMaps: () => Object.keys(worlds).map((name) => ({
            name,
        ...
        queryPublicReceipts: (params) => {
            ...
            return {
                mode: 'strict',
                receipts: raw.receipts.map((receipt) => toPublicReceipt(receipt, {
        ...
        getSessionMe: (guest_token) => resolveSessionMe(guest_token, 'expired_on_me'),
        getWorldPlayers: (map, query) => {
            const w = worlds[map];
            if (!w)
                return { error: 'unknown_map', status: 404 };
 ...
 */
function findPlayerByIdOnline(playerId) {
...
function sendTemPlayerFeedback(s, message) {
    if (!s.player)
        return;
...
function rookguardQuestInputFor(s) {
    return {
        tutorial: s.tutorial,
...
function playLoopFor(s) {
    ...
    if (s.currentMap === 'Rookguard')
        objective = rookguardQuestObjective(rookguardQuestInput);
    else if (!s.tutorial.gate)
        objective = 'Enter the High City gate';
    else if (!s.heraldMet)
        objective = 'Seek the High City herald in the southern plaza';
    else if (bloom.phase === 'signal' || bloom.phase === 'investigation')
        objective = 'Help resolve the Witness Moth Bloom above High City';
    else
        objective = 'Talk to the steward at guild hall';
    return {
        ...s.tutorial,
        gateOpen,
        objective,
        rookguardQuest: buildRookguardQuestProgress(rookguardQuestInput),
        onwardRoutes: buildOnwardRouteProgress(rookguardQuestInput, s.player ? getOnwardRouteReceiptProgress(s.player.id) : undefined),
        lastEvent: bloom.phase === 'idle' ? null : `witness_moth_bloom_${bloom.phase}`,
        ...(bloom.teaser ? { teaser: bloom.teaser } : {}),
    };
}
function sendLoopUpdate(s, event) {
    if (!s.player || s.ws.readyState !== WebSocket.OPEN)
        return;
    send(s.ws, ServerMessages.loopUpdate(event, playLoopFor(s)));
}
function toPublicSessionPlayer(s, includeDeadUntil = false) {
    return {
        ...toPublicPlayer(s.player, includeDeadUntil),
        loop: playLoopFor(s),
    };
}
function broadcastToMap(map, message, excludeConnId) {
...
function gatherNodePublic(node) {
    return {
...
function gatherStationPublic(station) {
    return {
...
function applyRespawnNow(s, now) {
    if (!s.player)
        return;
    const w = worldFor(s);
    ...
    send(s.ws, ServerMessages.worldState(s.currentMap, toPublicSessionPlayer(s, true), nearby, builderPreviewForSession(s)));
    s.respawnTimer = null;
}
function builderPreviewForSession(s) {
    return builderPreviewForMap(s.guestToken, s.currentMap, builderPreviewBindings);
}
function refreshWorldStateForGuestToken(guestToken) {
    for (const s of sessions.values()) {
        if (s.guestToken === guestToken)
...
function sendWorldStateRefresh(s) {
    if (!s.player || s.ws.readyState !== WebSocket.OPEN)
        return;
...
function applyMobDeath(s, mobType, now) {
    if (!s.player)
        return;
    const w = worlds[s.currentMap];
    if (!w)
    ...
    send(s.ws, ServerMessages.deathNotice(deathResult.respawn_in_ms, s.currentMap, w.map.spawn, deathResult.changed ? 'npc' : 'already_dead'));
}
function scheduleRespawnIfNeeded(s, now) {
    if (!s.player)
        return;
    if (s.player.status !== 'dead')
        return;
    if (s.player.dead_until_ms === null || s.player.dead_until_ms === undefined)
        return;
    ...
    ws.on('close', () => {
        ...
        if (gatherSystem && s.player) {
            ...
            if (g.state === 'gathering') {
                ...
                if (node) {
                    broadcastToMap(g.zone, ServerMessages.gatherNodeUpdate(gatherNodePublic(node)));
                }
            }
        }
...
function processSessionQueue(s, now) {
    ...
    if (timeoutOutcome.outcome === 'failed') {
        ...
        sendTemPlayerFeedback(s, 'Time expired before the human check was answered. Movement is slowed briefly — watch for the next prompt.');
    }
    ...
    while (s.queue.length && processed < 25) {
        ...
        switch (msg.type) {
            ...
            case 'login': {
                ...
                const isSovereignByName = SOVEREIGN_ENABLED &&
                ...
                send(s.ws, ServerMessages.loginAck(player_id, guest_token ?? '', name, true, undefined, authToken ? { token: authToken, expires_at: tokenExpiresAt } : undefined));
                break;
            }
            case 'enter_world': {
                if (!requireAuth(s))
                ...
                const itemInfos = playerItems.map((itemId) => toItemInfoFromPersist(itemId, persist.getItem.bind(persist)));
                ...
                if (gatherSystem) {
                    ...
                    if (gz) {
                        send(s.ws, ServerMessages.gatherSnapshot(Array.from(gz.nodes.values()).map(gatherNodePublic), Array.from(gz.stations.values()).map(gatherStationPublic)));
                    }
                }
            ...
            case 'tem_response': {
                ...
                if (out.outcome === 'passed') {
                    ...
                    if (rookguardTemCompleted)
                        sendLoopUpdate(s, 'rookguard_tem_complete');
                }
                    ...
                    sendTemPlayerFeedback(s, 'That answer did not match. Movement is slowed briefly — use the prompt and type AKALYNTH.');
                }
                break;
            }
            case 'chat': {
                ...
                if (DEBUG_MODE && msg.message.startsWith('/legendary')) {
                    ...
                    const playerItems = Array.from(inventory.get(s.player.id) ?? []).map((id) => toItemInfoFromPersist(id, persist.getItem.bind(persist)));
                    send(s.ws, ServerMessages.inventorySnapshot(playerItems));
                    // Broadcast confirmation via chat
                    broadcastToMap(s.currentMap, ServerMessages.chatBroadcast('system', 'System', `${s.player.name} acquired a legendary item!`));
                    break;
                }
            ...
            case 'kill_self': {
                ...
                send(s.ws, ServerMessages.deathNotice(deathResult.respawn_in_ms, s.currentMap, w.map.spawn, deathResult.changed ? 'test' : 'already_dead'));
                break;
            }
            case 'move_intent': {
                if (!requireWorld(s))
                ...
                if (res.ok) {
                    const stoneLandmark = s.currentMap === LEGEND_STONE_MAP ? legendStoneLandmark(w.map) : null;
                    ...
                    if (s.currentMap === 'Rookguard') {
                        ...
                        if (tile === TileCode.GateToAzura && !s.tutorial.complete) {
                            if (rookguardGateOpen(rookguardQuestInputFor(s))) {
                                ...
                                send(s.ws, ServerMessages.worldState(s.currentMap, toPublicSessionPlayer(s, true), nearbyAzura, builderPreviewForSession(s)));
                                ...
                                if (gatherSystem) {
                                    ...
                                    if (azuraGz) {
                                        send(s.ws, ServerMessages.gatherSnapshot(Array.from(azuraGz.nodes.values()).map(gatherNodePublic), Array.from(azuraGz.stations.values()).map(gatherStationPublic)));
                                    }
                                }
                ...
                send(s.ws, ServerMessages.moveResult(res.ok, finalX, finalY, res.reason, transferred ? s.currentMap : undefined));
                if (res.ok && !transferred)
                    broadcastToMap(s.currentMap, ServerMessages.playerMoved(s.player.id, finalX, finalY), s.connId);
            ...
            case 'runestone_cast': {
                ...
                if (s.lastRunestoneCastAtMs !== null && msgNow - s.lastRunestoneCastAtMs < RUNESTONE_COOLDOWN_MS) {
                    ...
                    if (s.runestoneCooldownWindowStartMs === null ||
                ...
                const resultMsg = ServerMessages.runestoneResult(msg.table_id, { id: s.player.id, name: s.player.name }, face, whisper);
                // Send to caster
                send(s.ws, resultMsg);
                // Broadcast to nearby players (within radius, on same map)
                for (const [otherConnId, other] of sessions) {
                    if (otherConnId === s.connId)
                        continue;
                    if (!other.inWorld)
                        continue;
                    if (other.currentMap !== s.currentMap)
                        continue;
                    if (!other.player)
                        continue;
            ...
            case 'drop_item': {
                if (!requireWorld(s))
                    break;
                const ok = handleDropItem(s.player.id, msg.item_id, s.currentMap, s.player.x, s.player.y);
                if (ok) {
            ...
            case 'pickup_item': {
                if (!requireWorld(s))
                    break;
                const ok = handlePickupItem(s.player.id, msg.item_id, s.currentMap, s.player.x, s.player.y);
                if (ok) {
                    // Send result to player
                    ...
                    const invItems = invIds.map((id) => toItemInfoFromPersist(id, persist.getItem.bind(persist), null));
            ...
            case 'attack_intent': {
                ...
                if (targetId.startsWith('mob:')) {
                    ...
                    if (hit.dead) {
                        ...
                        broadcastToMap(s.currentMap, ServerMessages.combatResolved(attackerId, targetId, 'kill', s.currentMap, mob.def.x, mob.def.y));
                        ...
                        if (!worldItems.has(s.currentMap))
                            worldItems.set(s.currentMap, new Map());
                ...
                const ctx = {
                    ...
                    adjustReputation: (playerId, delta) => {
                    ...
                    computeReceiptHash,
                    getProtectedItemId: (playerId) => protectedByPlayerId.get(playerId),
                    getRngCommitV1: (playerId) => {
                        for (const sess of sessions.values()) {
                            if (sess.player?.id === playerId) {
                ...
                if (result.success && result.map && result.defenderPos && result.droppedItemIds) {
                    ...
                    broadcastToMap(result.map, ServerMessages.combatResolved(attackerId, targetId, 'kill', result.map, result.defenderPos.x, result.defenderPos.y));
                    ...
                    for (const itemId of result.droppedItemIds) {
                        const item = persist.getItem(itemId);
                        broadcastToMap(result.map, ServerMessages.worldItemAdded(itemId, item?.item_type ?? 'unknown', result.defenderPos.x, result.defenderPos.y));
                    }
                    ...
                    if (defenderSession) {
                        ...
                        send(defenderSession.ws, ServerMessages.deathNotice(DEATH_RESPAWN_DELAY_MS, result.map, { x: worldState.map.spawn.x, y: worldState.map.spawn.y }, 'Killed by another player', { lost_items: lostItems }));
                    ...
                    send(s.ws, ServerMessages.combatRejected(result.reason));
                }
                break;
            }
            ...
            case 'get_chronicle': {
                ...
                const chronicleEvents = events.map((e) => {
                    ...
                    if (e.evidence_ref) {
                        try {
                            event.evidence_ref = JSON.parse(e.evidence_ref);
                        }
                        catch {
            ...
            case 'get_evidence': {
                ...
                const evidenceCtx = {
                    ...
                    getPlayerInventorySnapshot: (playerId, _timestamp) => {
                        const playerInv = inventory.get(playerId);
                        if (!playerInv || playerInv.size === 0)
                            return [];
                ...
                send(s.ws, ServerMessages.evidenceSnapshot(result.status, selfPlayerId, {
            ...
            case 'get_pressure_metrics': {
                ...
                const metricsResult = computePressureMetrics({ db: persist.db }, selfPlayerId, sinceDate.toISOString(), untilDate.toISOString());
                send(s.ws, ServerMessages.pressureMetricsSnapshot(selfPlayerId, sinceDate.toISOString(), untilDate.toISOString(), metricsResult.status, metricsResult.metrics, metricsResult.error_code));
                break;
            }
            // ====================================================================
            ...
            case 'declare_vocation': {
                ...
                const readyForCodex = s.currentMap === 'Rookguard' &&
            ...
            case 'inspect_player': {
                ...
                if (!target) {
                    send(s.ws, ServerMessages.playerInspect(msg.target_player_id, '', null, null, [], null, 'not_found'));
                    break;
                }
                ...
                send(s.ws, ServerMessages.playerInspect(target.id, target.name, identity.vocation, display_vocation, target.badges ?? [], mark));
                break;
            }
            case 'grant_sovereign_prefix': {
                if (!DEBUG_MODE)
                    break;
                if (!process.env.SOVEREIGN_PREFIX_DEBUG)
                    break; // Double-gate
                if (!requireAuth(s))
                    break;
            ...
            case 'pay_tithe': {
                ...
                audit.write({
                    ...
                    inputs: { amount, reason: 'temple_tithe' },
                    result: 'ok',
                });
            ...
            case 'grant_gold': {
                ...
                if (!target)
                    break;
                const amount = msg.amount;
                if (typeof amount !== 'number' || amount <= 0 || !Number.isInteger(amount) || amount > MAX_GOLD_AMOUNT)
                    break;
                // Emit credit receipt — treasury Map updated via reducer hook
                audit.write({
                    player_id: target.id,
                    action: WALLET_CREDIT_ACTION,
                    inputs: { amount, reason: 'debug_grant' },
                    result: 'ok',
                });
                break;
            }
            ...
            case 'start_work_contract': {
                if (!requireAuth(s) || !s.player)
                    break;
                const result = startContract(s.player.id, msg.contract_type, Date.now(), (r) => audit.write(r));
                if (result.ok) {
                    send(s.ws, ServerMessages.workContractStarted(result.contract_id, msg.contract_type, result.payout_gold, result.cooldown_seconds, result.min_duration_ms));
                }
                else {
                    send(s.ws, ServerMessages.workContractResult('', false, undefined, result.error));
                }
                break;
            }
            case 'work_tick': {
                ...
                if (tickResult.ok) {
                    send(s.ws, ServerMessages.workProgress(msg.contract_id, tickResult.ticks_observed, tickResult.ticks_required, tickResult.remaining_ms));
                    // Auto-complete if all gates passed
                    if (tickResult.ready_to_complete) {
                        const completeResult = completeContract(s.player.id, msg.contract_id, nowMs, (r) => audit.write(r));
                        send(s.ws, ServerMessages.workContractResult(msg.contract_id, completeResult.ok, completeResult.ok ? completeResult.credited_gold : undefined, completeResult.ok ? undefined : completeResult.error));
                        if (completeResult.ok) {
            ...
            case 'gather_intent': {
                ...
                const res = startGather(gatherSystem, s.player.id, s.currentMap, msg.node_id, s.player.x, s.player.y, Date.now());
                if (res.ok) {
                    ...
                    send(s.ws, ServerMessages.gatherResult(false, undefined, undefined, res.reason.toLowerCase()));
                }
                break;
            }
            case 'deliver_intent': {
                ...
                if (res.ok) {
                    ...
                    send(s.ws, ServerMessages.deliverResult(true, res.record.station_id, res.record.item_type, res.record.source_node_id, undefined, res.record.reward, res.record.refined));
                    ...
                    send(s.ws, ServerMessages.deliverResult(false, undefined, undefined, undefined, res.reason.toLowerCase()));
                }
                break;
            }
            case 'refine_intent': {
                ...
                const res = startRefine(gatherSystem, s.player.id, s.currentMap, msg.station_id, s.player.x, s.player.y, Date.now());
                if (res.ok) {
                    send(s.ws, ServerMessages.refineResult(true, res.station_id, res.complete_at_ms));
                }
                else {
                    send(s.ws, ServerMessages.refineResult(false, undefined, undefined, res.reason.toLowerCase()));
                }
                break;
            }
            // ========================================================================
            ...
            case 'buy_house': {
                ...
                broadcastToMap(updated.zone, ServerMessages.houseSold(updated.property_id, updated.plot_id, updated.zone, s.player.name, sellerName, updated.owner_history[updated.owner_history.length - 1]?.price ?? 0, updated.sale_count));
                break;
            }
            case 'list_house': {
                if (!requireAuth(s) || !s.player)
                ...
                send(s.ws, ServerMessages.propertyState(propertyToPublic(updated)));
                broadcastToMap(updated.zone, ServerMessages.propertyState(propertyToPublic(updated)), s.connId);
                break;
            }
            case 'unlist_house': {
                if (!requireAuth(s) || !s.player)
            ...
            case 'open_house_auction': {
                ...
                if (prop.status !== 'owned') {
                    send(s.ws, ServerMessages.propertyResult('open_house_auction', false, msg.property_id, prop.status === 'auctioning' ? 'already_auctioning' : 'already_listed'));
                    break;
                }
                ...
                send(s.ws, ServerMessages.propertyState(propertyToPublic(updated)));
                if (auction) {
                    const stateMsg = ServerMessages.propertyAuctionState(auction.property_id, auction.kind, auction.current_high, resolvePlayerName(auction.high_bidder_id), minNextBid(auction), auction.scheduled_close_ms);
                    send(s.ws, stateMsg);
                    broadcastToMap(updated.zone, stateMsg, s.connId);
                }
                break;
            }
            case 'place_house_bid': {
                ...
                audit.write({
                    player_id: bidder,
                    action: WALLET_DEBIT_ACTION,
                    inputs: { amount: msg.amount, reason: `auction_escrow:${auction.property_id}` },
                    result: 'ok',
                });
                ...
                if (priorBidder && priorAmount != null) {
                    audit.write({
                        ...
                        action: WALLET_CREDIT_ACTION,
                        inputs: { amount: priorAmount, reason: `auction_refund:${auction.property_id}` },
                        result: 'ok',
                    });
                    audit.write({
                        player_id: 'system',
                ...
                const prop = getProperty(msg.property_id);
                const stateMsg = ServerMessages.propertyAuctionState(updated.property_id, updated.kind, updated.current_high, resolvePlayerName(updated.high_bidder_id), minNextBid(updated), updated.scheduled_close_ms);
                ...
                broadcastToMap(prop.zone, stateMsg, s.connId);
                break;
            }
            ...
            case 'talk_to_npc': {
                ...
                if (msg.npc_id === 'azura_herald' && !s.heraldMet) {
                    s.heraldMet = true;
                    const bloomStart = startWitnessMothBloom(witnessMothBloom, { player_id: s.player.id, map: s.currentMap, now_ms: Date.now() }, (receipt) => audit.write(receipt));
            ...
            case 'use_skill': {
                ...
                if (GUILD_ENABLED && msg.skill_id === 'guild:contribute') {
                    ...
                    const milestoneReached = GUILD_TREASURY_MILESTONES.find((m) => prevTreasury < m && guildTreasuryTotal >= m) ?? null;
                    audit.write({
                        player_id: s.player.id,
                ...
                if (HOUSES_ENABLED && (msg.skill_id === 'house:grant' || msg.skill_id === 'house:revoke')) {
                    ...
                    const set = houseAccess.get(propertyId) ?? new Set();
                    if (granting)
                        set.add(targetId);
                    else
                        set.delete(targetId);
                ...
                if (HOUSES_ENABLED && msg.skill_id.startsWith('house:store:')) {
                    ...
                    if (protectedByPlayerId.get(s.player.id) === itemId)
                        protectedByPlayerId.delete(s.player.id);
                    const stored = houseStorage.get(propertyId) ?? new Set();
                    stored.add(itemId);
                ...
                if (HOUSES_ENABLED && msg.skill_id.startsWith('house:retrieve:')) {
                    ...
                    if (!inventory.has(s.player.id))
                        inventory.set(s.player.id, new Set());
                ...
                if (msg.skill_id.startsWith('shop:')) {
                    ...
                    audit.write({
                        player_id: s.player.id,
                        action: WALLET_DEBIT_ACTION,
                        inputs: { amount: shopItem.price, reason: 'action_cost:shop_purchase' },
                        result: 'ok',
                    });
                    ...
                    const shopInvItems = shopInvIds.map((id) => toItemInfo(id, persist.getItem(id)?.item_type ?? (id === shopItemId ? shopItem.item_type : 'unknown'), null));
                ...
                if (msg.skill_id.startsWith('event:witness_moth_bloom:')) {
                    ...
                    const result = handleWitnessMothBloomSkillIntent(witnessMothBloom, {
                        player_id: s.player.id,
                        map: s.currentMap,
                ...
                const skillCtx = {
                    ...
                    onSkillResolved: (skillId) => {
                        if (skillId.startsWith('route:'))
                            sendLoopUpdate(s, 'onward_route_progress');
                    },
                    mintItemToInventory: (itemType, meta, reason, source) => mintItemToInventory(s.player.id, itemType, meta, reason, source),
                    syncInventory: () => {
                        const invIds = getPlayerInventoryIds(s.player.id);
                        const invItems = invIds.map((id) => toItemInfoFromPersist(id, persist.getItem.bind(persist), null));
                        send(s.ws, ServerMessages.inventorySnapshot(invItems));
...
setInterval(() => {
    ...
    if (gatherSystem) {
        ...
        for (const c of gatherEffects.completed) {
            ...
            if (node)
                broadcastToMap(c.zone, ServerMessages.gatherNodeUpdate(gatherNodePublic(node)));
        }
        const refinedByPlayer = new Map();
        ...
        for (const r of gatherEffects.respawned) {
            ...
            if (node)
                broadcastToMap(r.zone, ServerMessages.gatherNodeUpdate(gatherNodePublic(node)));
        }
        for (const gs of sessions.values()) {
            if (!gs.inWorld || !gs.player)
                continue;
            ...
            if (g.state === 'gathering') {
                ...
                if (pct !== null)
                    send(gs.ws, ServerMessages.gatherProgress(g.node_id, pct));
            }
            else if (g.state === 'refining') {
                const pct = refineProgressPct(gatherSystem, gs.player.id, now);
                if (pct !== null)
                    send(gs.ws, ServerMessages.refineProgress(g.station_id, pct));
            }
        }
    }
    ...
    if (now - lastAuctionScanMs >= AUCTION_SCAN_INTERVAL_MS) {
        ...
        for (const st of settlements) {
            broadcastToMap(st.zone, ServerMessages.houseAuctionSettled(st.property_id, st.plot_id, st.zone, resolvePlayerName(st.winner_id), resolvePlayerName(st.seller_id), st.price, st.sale_count));
        }
    }
}, TICK_MS);
...
setInterval(() => {
    ...
    for (const s of sessions.values()) {
        ...
        if (!playerInventory || playerInventory.size === 0)
            continue;
...
setInterval(() => {
    ...
    for (const map of ['Rookguard', 'Azura']) {
...
setInterval(() => {
    const now = Date.now();
    for (const map of ['Rookguard', 'Azura']) {
        const aggressors = getMobsForMap(map).filter((m) => m.dead_until_ms === null && AGGRESSIVE_MOB_TYPES.has(m.def.mob_type));
        if (aggressors.length === 0)
            continue;
        for (const s of sessions.values()) {
            if (!s.player || !s.inWorld || s.currentMap !== map)
                continue;
            if (s.player.status === 'dead')
                continue;
