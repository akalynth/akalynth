...
import { ASHGLASS_EVIDENCE_RECOVERED_ACTION, DREAM_FRAGMENT_ANCHORED_ACTION, DREAM_GATE_ARRIVAL_RECORDED_ACTION, FORGEHOLD_COMPONENT_PAYOUT_CREDITED_ACTION, FORGEHOLD_COMPONENT_SETTLED_ACTION, FORGEHOLD_ECONOMY_QUOTED_ACTION, HEARTFORGE_GATE_PREPARED_ACTION, DREAM_GATE_SEAL_PREPARED_ACTION, DREAM_GATE_TRAVERSAL_AUTHORIZED_ACTION, PLAYER_REPORTED_ACTION, ROUTE_ABUSE_NOTES_REVIEWED_ACTION, ROUTE_SURVEYED_ACTION, SOULSTEEL_COMPONENT_MINTED_ACTION, SOULSTEEL_REFINEMENT_AUTHORIZED_ACTION, SOULSTEEL_STABILIZED_ACTION, DREAM_GATE_INTERPRETED_ACTION, FORGEHOLD_ASHGLASS_RAVINE_EVIDENCE_RECOVERED_ACTION, FORGEHOLD_CARAVAN_EVIDENCE_RECOVERED_ACTION, FORGEHOLD_MILEPOST_EVIDENCE_RECOVERED_ACTION, FORGEHOLD_SHIPMENT_INVESTIGATED_ACTION, ROOKGUARD_CANAL_FISHED_ACTION, } from '../../../../packages/shared/skills.js';
import { createHash } from 'node:crypto';
// ============================================================================
// skill_inspect: Snapshot player state (read-only, redacted)
// ============================================================================
export function handleInspect(ctx, targetId) {
...
// ============================================================================
export function handlePingTem(ctx) {
...
// ============================================================================
export function handleRequestRecap(ctx) {
...
// ============================================================================
export function handleReport(ctx, targetId) {
    const target = ctx.findPlayerOnline(targetId);
    if (!target) {
        return { success: false, reason: 'target_not_found' };
    }
...
// ============================================================================
export function handleGiftGold(ctx, targetId) {
    if (targetId === ctx.playerId)
        return { success: false, reason: 'invalid_target' };
    const target = ctx.findPlayerOnline(targetId);
    if (!target)
        return { success: false, reason: 'target_not_found' };
    if (!ctx.debitWallet || !ctx.creditWalletForPlayer)
        return { success: false, reason: 'invalid_target' };
    const amount = 1;
    ...
    if (!debit.ok)
        return { success: false, reason: 'invalid_skill', payload: { error: 'insufficient_gold' } };
...
// ============================================================================
export function handleRookguardCanalFishing(ctx) {
...
// ============================================================================
export function handleRouteSurvey(ctx, route) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (route === 'forgehold' && routeProgress?.forgeholdSurveyed)
        return { success: false, reason: 'invalid_target' };
    if (route === 'moonspire' && routeProgress?.moonspireSurveyed)
        return { success: false, reason: 'invalid_target' };
...
export function handleSoulsteelStabilization(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    ...
    if (routeProgress.soulsteelStabilized)
        return { success: false, reason: 'invalid_target' };
...
export function handleForgeholdEconomyQuote(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    ...
    if (routeProgress.forgeholdEconomyQuoted)
        return { success: false, reason: 'invalid_target' };
...
export function handleForgeholdComponentSettlement(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.soulsteelComponentMinted)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.forgeholdComponentSettled)
        return { success: false, reason: 'invalid_target' };
...
export function handleForgeholdComponentPayout(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.forgeholdComponentSettled)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.forgeholdComponentPayoutCredited)
        return { success: false, reason: 'invalid_target' };
    if (!ctx.creditWallet)
        return { success: false, reason: 'invalid_target' };
...
export function handleDreamGateInterpretation(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.moonspireSurveyed)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.dreamGateInterpreted)
        return { success: false, reason: 'invalid_target' };
...
export function handleDreamFragmentAnchor(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    ...
    if (routeProgress.dreamFragmentAnchored)
        return { success: false, reason: 'invalid_target' };
...
export function handleRouteSafetyReview(ctx, route) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
...
export function handleHeartforgeGatePreparation(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.forgeholdAbuseNotesReviewed)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.heartforgeGatePrepared)
        return { success: false, reason: 'invalid_target' };
...
export function handleAshglassEvidenceRecovery(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.heartforgeGatePrepared)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.ashglassEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
...
export function handleSoulsteelRefinementAuthorization(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.ashglassEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.soulsteelRefinementAuthorized)
        return { success: false, reason: 'invalid_target' };
...
export function handleSoulsteelComponentMint(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.soulsteelRefinementAuthorized)
...
export function handleDreamGateSealPreparation(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.dreamGateAbuseNotesReviewed)
        return { success: false, reason: 'invalid_target' };
    const preparedAt = new Date().toISOString();
...
export function handleDreamGateTraversalAuthorization(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.dreamGateSealPrepared)
        return { success: false, reason: 'invalid_target' };
    const authorizedAt = new Date().toISOString();
...
export function handleDreamGateArrivalRecord(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.dreamGateTraversalAuthorized)
        return { success: false, reason: 'invalid_target' };
...
export function handleForgeholdMilepostEvidence(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.forgeholdSurveyed)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.forgeholdMilepostEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
    const recoveredAt = new Date().toISOString();
...
export function handleForgeholdCaravanEvidence(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.forgeholdMilepostEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.forgeholdCaravanEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
    const recoveredAt = new Date().toISOString();
    const authorityGuard = actIIEvidenceGuard();
    ctx.audit({
...
export function handleForgeholdAshglassRavineEvidence(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.forgeholdCaravanEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
    if (routeProgress.forgeholdAshglassRavineEvidenceRecovered)
        return { success: false, reason: 'invalid_target' };
    const recoveredAt = new Date().toISOString();
    const authorityGuard = actIIEvidenceGuard();
    ctx.audit({
...
export function handleForgeholdShipmentInvestigation(ctx) {
    if (!ctx.onwardRoutesAvailable)
        return { success: false, reason: 'invalid_target' };
    const routeProgress = ctx.getOnwardRouteProgress?.();
    if (!routeProgress?.forgeholdSurveyed)
    ...
    if (routeProgress.forgeholdShipmentInvestigated)
        return { success: false, reason: 'invalid_target' };
