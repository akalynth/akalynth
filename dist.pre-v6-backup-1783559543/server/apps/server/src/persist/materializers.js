 ...
 */
export function generateItemId(receiptHash) {
    // Strip "blake3:" prefix
...
// Main Materializer
// ============================================================================
export function materialize(db, receipt, offsetAfterLine) {
    ...
    db.transaction(() => {
        ...
        const updateMeta = db.prepare('INSERT OR REPLACE INTO _meta (key, value) VALUES (?, ?)');
...
// Player Handlers
// ============================================================================
function handlePlayerCreated(db, receipt, receiptHash) {
    // Extract player data from receipt
    const playerId = receipt.actor_id;
    const name = receipt.inputs?.name ?? `Guest_${playerId.slice(-4)}`;
...
function handlePlayerRenamed(db, receipt, _receiptHash) {
    const playerId = receipt.actor_id;
    const newName = receipt.inputs?.new_name;
    if (!newName)
        return;
    const stmt = db.prepare(`
...
function handleHouseItemStored(db, receipt, receiptHash) {
    const itemId = receipt.inputs?.item_id;
    const propertyId = receipt.inputs?.property_id;
...
function handleHouseItemRetrieved(db, receipt, _receiptHash) {
    const itemId = receipt.inputs?.item_id;
    if (!itemId)
        return;
...
function handleGuildContribution(db, receipt, receiptHash) {
 ...
 */
function handleOriginActSealed(db, receipt, receiptHash) {
    const playerId = receipt.actor_id;
    const inputs = receipt.inputs ?? {};
    const triggerAction = inputs.trigger_action;
    const timestamp = receipt.timestamp;
    if (!triggerAction)
        return;
  ...
  `).run(receiptHash, triggerAction, timestamp, playerId, timestamp);
    ...
    insertChronicleEvent(db, playerId, 'origin_sealed', timestamp, 'origin_act_sealed', receiptHash, null, null, null, null, { trigger_action: triggerAction });
}
// ============================================================================
// Death Handler
// ============================================================================
function handleDeath(db, receipt, receiptHash) {
    const playerId = receipt.actor_id;
    const timestamp = receipt.timestamp;
    const inputs = receipt.inputs ?? {};
    const zone = inputs.zone ?? inputs.map ?? 'unknown';
    const x = inputs.x ?? inputs.pos?.x ?? 0;
    const y = inputs.y ?? inputs.pos?.y ?? 0;
    const cause = inputs.cause ?? 'unknown';
...
// Reputation Handler
// ============================================================================
function handleReputationEvent(db, receipt, receiptHash) {
    const playerId = receipt.actor_id;
    const timestamp = receipt.timestamp;
    const inputs = receipt.inputs ?? {};
    const eventType = inputs.type ?? receipt.action;
    const delta = inputs.delta ?? inputs.penalty ?? 0;
    const witnesses = inputs.witnesses ? JSON.stringify(inputs.witnesses) : null;
...
// ============================================================================
function handleWorldObjectSpawned(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const objectId = inputs.object_id ?? inputs.item_id;
    if (!objectId)
        return;
    const objectType = inputs.object_type ?? inputs.type ?? 'item';
    const zone = inputs.zone ?? inputs.map ?? 'unknown';
    const x = inputs.x ?? 0;
    const y = inputs.y ?? 0;
    const createdAt = receipt.timestamp;
    const decayAt = inputs.decay_at ?? null;
...
function handleWorldObjectTransferred(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const objectId = inputs.object_id ?? inputs.item_id;
    if (!objectId)
        return;
    const newOwner = inputs.new_owner ?? receipt.actor_id;
...
function handleWorldObjectRemoved(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const objectId = inputs.object_id ?? inputs.item_id;
    if (!objectId)
        return;
...
function readPropertyOwnerHistory(db, propertyId) {
    const existing = db
    ...
    if (!existing?.owner_history)
        return [];
    try {
        return JSON.parse(existing.owner_history);
    }
    catch {
        return [];
    }
}
function handlePropertyCreated(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    const zone = inputs.zone;
    const plotId = inputs.plot_id;
    if (!propertyId || !zone || !plotId)
        return;
  ...
  `);
    stmt.run(propertyId, zone, plotId, Number(inputs.x ?? 0), Number(inputs.y ?? 0), Number(inputs.width ?? 0), Number(inputs.height ?? 0), inputs.district ?? null, Number(inputs.primary_price_gold ?? 0), receiptHash, receiptHash, receipt.timestamp);
}
function handlePropertyListed(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    const price = inputs.price;
    if (!propertyId || typeof price !== 'number')
        return;
...
function handlePropertyUnlisted(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    if (!propertyId)
        return;
...
function handlePropertyPurchased(db, receipt, receiptHash) {
    ...
    if (!propertyId || !buyer)
        return;
    const price = typeof inputs.price === 'number' ? inputs.price : 0;
...
function handlePropertyTransferred(db, receipt, receiptHash) {
    // Resale: seller → buyer (actor_id = buyer).
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    const sellerId = inputs.seller_id;
    const buyer = receipt.actor_id;
    if (!propertyId || !sellerId || !buyer)
        return;
    const price = typeof inputs.price === 'number' ? inputs.price : 0;
    const history = readPropertyOwnerHistory(db, propertyId);
...
// ----------------------------------------------------------------------------
function handlePropertyAuctionOpened(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    const kind = inputs.kind;
    const minBid = inputs.min_bid;
    const minIncrement = inputs.min_increment_gold;
    if (!propertyId || (kind !== 'primary' && kind !== 'resale'))
        return;
    if (typeof minBid !== 'number' || typeof minIncrement !== 'number')
        return;
    const sellerId = kind === 'resale' ? receipt.actor_id : null;
    const scheduledClose = typeof inputs.scheduled_close_ms === 'number' ? inputs.scheduled_close_ms : null;
...
function handlePropertyBid(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    const amount = inputs.amount;
    const bidder = receipt.actor_id;
    if (!propertyId || typeof amount !== 'number' || !bidder)
        return;
...
function handlePropertyBidRefunded(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    if (!propertyId)
        return;
    ...
    db.prepare(`UPDATE property_auctions SET last_receipt = ? WHERE property_id = ?`).run(receiptHash, propertyId);
}
function handlePropertyAuctionSettled(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    if (!propertyId)
        return;
    const kind = inputs.kind;
    const winnerId = inputs.winner_id ?? null;
    const sellerId = inputs.seller_id ?? null;
    const price = typeof inputs.price === 'number' ? inputs.price : 0;
    if (winnerId) {
        const history = readPropertyOwnerHistory(db, propertyId);
        if (kind === 'primary') {
...
function handlePropertyAuctionCancelled(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const propertyId = inputs.property_id;
    if (!propertyId)
        return;
 ...
 */
function handleItemMinted(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
 ...
 */
function handleItemAddedToInventory(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const itemId = inputs.item_id;
    const slot = inputs.slot ?? null;
    if (!itemId)
        return;
  ...
  `).run(receipt.actor_id, `Guest_${receipt.actor_id.slice(-4)}`, receipt.timestamp, receiptHash);
    ...
    const stubItemType = inputs.item_type ?? 'unknown';
    db.prepare(`
    INSERT OR IGNORE INTO items (item_id, item_type, created_at, genesis_receipt, meta_json)
    VALUES (?, ?, ?, ?, ?)
 ...
 */
function handleItemRemovedFromInventory(db, receipt, _receiptHash) {
    const inputs = receipt.inputs ?? {};
    const itemId = inputs.item_id;
    if (!itemId)
        return;
 ...
 */
function handleItemDroppedToWorld(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const itemId = inputs.item_id;
    const zone = inputs.zone;
    const x = inputs.x;
    const y = inputs.y;
    const decayAt = inputs.decay_at ?? null;
    if (!itemId || !zone || x === undefined || y === undefined)
        return;
    // Projection repair: remove from inventory if still present
    // (ensures exclusivity even under weird receipt sequences)
    db.prepare(`DELETE FROM inventory_items WHERE item_id = ?`).run(itemId);
    // Get item_type from items table
    const item = db.prepare(`SELECT item_type FROM items WHERE item_id = ?`).get(itemId);
    ...
    if (existing) {
        ...
        const history = JSON.parse(existing.owner_history);
 ...
 */
function handleItemPickedUpFromWorld(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const itemId = inputs.item_id;
    if (!itemId)
        return;
 ...
 */
function handleLegendaryHeatChanged(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const itemId = inputs.item_id;
    const newHeat = inputs.new_heat;
    if (!itemId || typeof newHeat !== 'number')
        return;
    db.prepare(`
...
function handlePlayerHeatChanged(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const newScore = inputs.new_score;
...
function handleHeatPenaltyApplied(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const durationMs = inputs.duration_ms;
    const timestampMs = Date.parse(receipt.timestamp);
    if (!Number.isFinite(durationMs) || !Number.isFinite(timestampMs))
        return;
...
function handleHeatTemEscalation(db, receipt, receiptHash) {
    const timestampMs = Date.parse(receipt.timestamp);
...
function handleTemChallengeFailed(db, receipt, receiptHash) {
    const timestampMs = Date.parse(receipt.timestamp);
    if (!Number.isFinite(timestampMs))
        return;
...
function handleThrottleApplied(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const durationMs = typeof inputs.duration_ms === 'number' ? inputs.duration_ms : THROTTLE_DURATION_MS;
    const timestampMs = Date.parse(receipt.timestamp);
    if (!Number.isFinite(durationMs) || !Number.isFinite(timestampMs))
        return;
...
function handleKickApplied(db, receipt, receiptHash) {
    db.prepare(`
    INSERT INTO player_anticheat_enforcement
    (player_id, warn_count, tem_failed_count, throttle_count, kick_count, throttle_until_ms, updated_at, last_receipt)
    VALUES (?, 0, 0, 0, 1, NULL, ?, ?)
...
function handleWarnIssued(db, receipt, receiptHash) {
    db.prepare(`
    INSERT INTO player_anticheat_enforcement
    (player_id, warn_count, tem_failed_count, throttle_count, kick_count, throttle_until_ms, updated_at, last_receipt)
    VALUES (?, 1, 0, 0, 0, NULL, ?, ?)
 ...
 */
function handleInventorySlotChanged(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const itemId = inputs.item_id;
 ...
 */
function handlePlayerReported(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const caseId = inputs.case_id;
    const reporterId = inputs.reporter_id;
    const targetId = inputs.target_id;
    const timestamp = inputs.timestamp ?? receipt.timestamp;
    if (!caseId || !reporterId || !targetId)
        return;
    // INSERT OR IGNORE (idempotent via UNIQUE receipt_hash)
    db.prepare(`
 ...
 */
function handleModerationResolved(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const caseId = inputs.case_id;
    const resolution = inputs.resolution;
 ...
 */
function handleCharacterCreate(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    ...
    const nameLower = inputs.name_lower;
    const timestamp = receipt.timestamp;
    if (!playerId || !name || !nameLower)
        return;
 ...
 */
function handleAuthTokenIssue(_db, _receipt, _receiptHash) {
 ...
 */
function handleNpcTalked(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const npcId = inputs.npc_id;
    const tier = inputs.tier;
    const playerId = receipt.actor_id;
    if (!npcId || !tier || !playerId)
        return;
    db.prepare(`
...
// World Events v0
// ============================================================================
function handleWorldEventStarted(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const eventId = inputs.event_id;
    const map = inputs.map;
    const phase = inputs.phase ?? 'signal';
    if (!eventId || !map)
        return;
    db.prepare(`
...
function handleWorldEventEvidenceRecovered(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const eventId = inputs.event_id;
    const map = inputs.map;
    const evidenceId = inputs.evidence_id;
    if (!eventId || !map || !evidenceId)
        return;
    const row = db.prepare(`SELECT evidence_json FROM world_events WHERE event_id = ?`).get(eventId);
...
function handleWorldEventContribution(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const eventId = inputs.event_id;
    const map = inputs.map;
    const contributionId = inputs.contribution_id;
    if (!eventId || !map || !contributionId)
        return;
    const row = db.prepare(`SELECT phase, contributions_json FROM world_events WHERE event_id = ?`).get(eventId);
    ...
    contributions[contributionId] = {
        ...
        player_id: receipt.actor_id,
        recorded_at_ms: safeTimestampMs(receipt.timestamp),
    };
    if (!row) {
        db.prepare(`
      INSERT INTO world_events (
    ...
    db.prepare(`
    UPDATE world_events
    SET phase = 'investigation', contributions_json = ?, last_receipt = ?
    WHERE event_id = ?
  `).run(JSON.stringify(contributions), receiptHash, eventId);
}
function handleWorldEventResolved(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const eventId = inputs.event_id;
    const map = inputs.map;
    if (!eventId || !map)
        return;
    const outcome = inputs.outcome ?? null;
    const row = db.prepare(`SELECT contributions_json, evidence_json, teaser_json FROM world_events WHERE event_id = ?`).get(eventId);
...
function handleWorldEventTeaserUnlocked(db, receipt, receiptHash) {
    const inputs = receipt.inputs ?? {};
    const eventId = inputs.event_id;
    const map = inputs.map;
    const teaserId = inputs.teaser_id;
    if (!eventId || !map || !teaserId)
        return;
    const row = db.prepare(`SELECT contributions_json, evidence_json FROM world_events WHERE event_id = ?`).get(eventId);
    const teaser = {
        id: teaserId,
        unlocked: true,
...
function parseWorldEventJsonRecord(raw) {
    if (!raw)
        return {};
    try {
...
function safeTimestampMs(timestamp) {
 ...
 */
function insertChronicleEvent(db, playerId, kind, timestamp, sourceAction, receiptHash, zone = null, x = null, y = null, entityId = null, details = {}, evidenceRef = null) {
 ...
 */
function buildEvidenceRef(chronicleEventId, receiptHash) {
 ...
 */
function updateEvidenceRef(db, eventId, evidenceRef) {
    db.prepare(`
 ...
 */
function findCausingDeathEvent(db, playerId, beforeOrAt) {
 ...
 */
export function materializeChronicle(db, receipt, receiptHash) {
    const action = ACTION_ALIASES[receipt.action] ?? receipt.action;
    ...
    switch (action) {
        ...
        case RECEIPT_ACTIONS.PLAYER_CREATED: {
            const name = inputs.name ?? `Guest_${playerId.slice(-4)}`;
        ...
        case RECEIPT_ACTIONS.CHARACTER_CREATE: {
            if (receipt.result !== 'ok')
                break; // Only chronicle successful creations
            const charPlayerId = inputs.player_id;
            const name = inputs.name;
            if (!charPlayerId || !name)
                break;
            // entity_id: null (player_id is the entity)
        ...
        case RECEIPT_ACTIONS.DEATH: {
            const zone = inputs.zone ?? inputs.map ?? null;
            const x = inputs.x ?? inputs.pos?.x ?? null;
            const y = inputs.y ?? inputs.pos?.y ?? null;
            const cause = inputs.cause ?? 'unknown';
            const killerId = inputs.killer_id ?? null;
        ...
        case 'combat_resolved': {
            const targetId = inputs.target_player_id;
            const zone = inputs.map ?? null;
        ...
        case RECEIPT_ACTIONS.ITEM_ADDED_TO_INVENTORY: {
            ...
            const item = db.prepare(`SELECT meta_json FROM items WHERE item_id = ?`).get(itemId);
        ...
        case RECEIPT_ACTIONS.ITEM_REMOVED_FROM_INVENTORY: {
            const itemId = inputs.item_id;
            const reason = inputs.reason ?? 'unknown';
            if (!itemId)
                break;
        ...
        case RECEIPT_ACTIONS.PROPERTY_TRANSFERRED: {
            const propertyId = inputs.property_id;
            if (!propertyId || !playerId)
                break;
            const price = typeof inputs.price === 'number' ? inputs.price : 0;
            const fromSeller = inputs.seller_id ?? null;
        ...
        case RECEIPT_ACTIONS.WORLD_EVENT_TEASER_UNLOCKED: {
            ...
            if (!eventId || !playerId)
                break;
            const zone = inputs.map ?? null;
            const phase = inputs.phase ?? null;
            const evidenceId = inputs.evidence_id ?? null;
            const contributionId = inputs.contribution_id ?? null;
            const outcome = inputs.outcome ?? null;
            const teaserId = inputs.teaser_id ?? null;
