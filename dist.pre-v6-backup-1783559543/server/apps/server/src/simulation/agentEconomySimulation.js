import { computeEventHash, computeInputsHash, computeOutputsHash, GENESIS_MARKER, } from '@akalynth/coordination-kernel';
import { ACTION_GOLD_COST, PRESENCE_ENTERED_ACTION, PRESENCE_LINGERED_ACTION, PRESENCE_LINGER_THRESHOLD_MS, PRESENCE_OBSERVED_ACTION, PRESENCE_OBSERVE_THRESHOLD_MS, PROPERTY_AUCTION_OPENED_ACTION, PROPERTY_AUCTION_SETTLED_ACTION, PROPERTY_BID_ACTION, PROPERTY_BID_REFUNDED_ACTION, PROPERTY_CREATED_ACTION, PROPERTY_LISTED_ACTION, PROPERTY_PURCHASED_ACTION, PROPERTY_TRANSFERRED_ACTION, WALLET_CREDIT_ACTION, WALLET_DEBIT_ACTION, WORK_CONTRACT_SCHEDULE, } from '../../../../packages/shared/types.js';
...
import { handleAttackIntent } from '../world/combat.js';
import { applyDeath } from '../world/death.js';
import { tryMove } from '../world/movement.js';
import { spawnMobLoot } from '../world/mobs.js';
import { buildNpcDialogue, getNpcDef, getNpcIntent, resolveDialogueTier } from '../world/npcs.js';
import { applyReceiptToProperty, clearPropertyProjection, ensurePropertiesSeeded, getAllProperties, getMarketListings, } from '../world/property.js';
import { applyReceiptToTreasury, canAfford, clearTreasuryProjection, debitForAction, getGoldBalance, } from '../world/treasury.js';
import { applyReceiptToWorkContracts, clearWorkContractsProjection, completeContract, recordTick, startContract, } from '../world/work_contracts.js';
import { applyReceiptToPresence, clearPresenceProjection, getCurrentPlace, onPlayerMoved, onPresenceTick, registerMapPlaces, } from '../world/presence.js';
import { WITNESS_MOTH_BLOOM_EVIDENCE_PREFIX, WITNESS_MOTH_BLOOM_SKILL_PREFIX, createWitnessMothBloomRuntime, handleWitnessMothBloomSkillIntent, startWitnessMothBloom, } from '../world/world-events.js';
const DEFAULT_AGENTS = [
    { id: 'sim:worker:1', name: 'Sim Worker 1', role: 'worker', startingGold: 0 },
    { id: 'sim:homesteader:1', name: 'Sim Homesteader 1', role: 'homesteader', startingGold: 520 },
    { id: 'sim:merchant:1', name: 'Sim Merchant 1', role: 'merchant', startingGold: 1600 },
    { id: 'sim:merchant:2', name: 'Sim Merchant 2', role: 'merchant', startingGold: 4000 },
];
export function runAgentEconomySimulation(input) {
    const seed = input.seed ?? 1;
    const days = input.days ?? 2;
    const receiptState = { lastEventHash: null, sequence: 0 };
    const receipts = [];
    const steps = [];
    const npcDialogues = [];
    const inventoryByAgent = new Map();
    ...
    const agents = (input.agents ?? DEFAULT_AGENTS).map((a, idx) => {
    ...
    for (const agent of agents) {
        const startingGold = input.agents?.find((a) => a.id === agent.id)?.startingGold
            ...
            ?? 0;
        if (startingGold > 0) {
            emit(agent.id, WALLET_CREDIT_ACTION, { amount: startingGold, reason: 'debug_grant' }, nowMs);
        }
    }
    for (let day = 0; day < days; day += 1) {
        ...
        for (const agent of agents) {
            ...
            if (inspect.ok)
                action = `${action}+inspect_player`;
            if (loot)
                action = `${action}+loot_pickup`;
            steps.push({
                step: stepNo,
                tick_ms: nowMs,
                agent_id: agent.id,
...
function runNpcDialogueTrainingPhase(agents) {
    const samples = [];
    const configs = [
        { agentId: 'sim:worker:1', npcId: 'rookguard_guide', placeId: 'rookguard:plaza' },
        { agentId: 'sim:merchant:1', npcId: 'azura_herald', placeId: 'azura' },
        { agentId: 'sim:merchant:2', npcId: 'azura_steward', placeId: 'azura' },
    ];
...
function runWorldEventTrainingPhase(emit, nowMs) {
    ...
    for (let i = 0; i < skillIds.length; i += 1) {
        handleWitnessMothBloomSkillIntent(runtime, { player_id: actor, map: 'Azura', skill_id: skillIds[i], now_ms: nowMs + i + 1 }, write);
    }
}
function runCombatTrainingPhase(agents, nowMs, emit, inventoryByAgent, itemById, worldItems, reputationByAgent) {
    ...
    for (const [agentId, items] of inventoryByAgent)
        inventory.set(agentId, new Set(items));
    handleAttackIntent({
    ...
    for (const [agentId, items] of inventory)
        inventoryByAgent.set(agentId, [...items]);
}
function ensureCombatInventory(defender, nowMs, emit, inventoryByAgent, itemById) {
...
function runAuctionTrainingPhase(emit, nowMs) {
    ...
    emit(firstBidderId, WALLET_DEBIT_ACTION, {
        amount: minBid,
        reason: `auction_escrow:${property.property_id}`,
    }, nowMs + 2);
    ...
    emit(firstBidderId, WALLET_CREDIT_ACTION, {
        amount: minBid,
        reason: `auction_refund:${property.property_id}`,
    }, nowMs + 5);
...
function ensureGoldForAuctionBid(agentId, amount, nowMs, emit) {
...
function makeReceiptEmitter(state, receipts, apply) {
    return (actorId, action, inputs, nowMs) => {
...
function seedProperties(map, emit) {
...
function completeWorkLoop(agentId, contractType, nowMs, emit) {
...
function buyFirstAffordableProperty(agentId, emit, nowMs) {
    const property = getAllProperties()
        .filter((p) => p.status === 'unowned')
        .sort((a, b) => a.primary_price_gold - b.primary_price_gold)[0];
    if (!property || property.primary_price_gold <= 0 || !canAfford(agentId, property.primary_price_gold))
        return false;
    emit(agentId, WALLET_DEBIT_ACTION, {
        amount: property.primary_price_gold,
        reason: `property_purchase:${property.property_id}`,
    }, nowMs);
...
function runMerchantAction(agentId, emit, nowMs) {
    ...
    if (listing?.listed_price_gold !== null && listing?.owner_player_id && canAfford(agentId, listing.listed_price_gold)) {
        emit(agentId, WALLET_DEBIT_ACTION, {
            ...
            reason: `property_transfer:${listing.property_id}`,
        }, nowMs);
        emit(listing.owner_player_id, WALLET_CREDIT_ACTION, {
            amount: listing.listed_price_gold,
            reason: `property_sale:${listing.property_id}`,
        }, nowMs + 1);
    ...
    if (!owned && buyFirstAffordableProperty(agentId, emit, nowMs))
        return { action: 'buy_property', accepted: true };
...
function mintAndPickupLoot(agent, itemType, nowMs, emit, worldLoot, inventoryByAgent, itemById) {
    ...
    const loot = spawnMobLoot(agent.id, itemType, agent.map, agent.player.x, agent.player.y, {
        writeReceipt: (input) => emit(input.actor_id ?? input.player_id ?? agent.id, input.action, input.inputs, nowMs),
        computeReceiptHash,
        generateItemId,
        meta,
    });
...
function moveAgentAcrossWorld(agent, maps, seed) {
...
function summarizeSimulation(agents, steps, receipts, inventoryByAgent, npcDialogues) {
    const finalGold = {};
    const owned = {};
    const inventory = {};
    for (const agent of agents) {
...
function sumReceiptAmounts(receipts, action) {
    return receipts
        .filter((r) => r.action === action)
        .reduce((sum, r) => sum + (typeof r.inputs.amount === 'number' ? r.inputs.amount : 0), 0);
}
export function assertAgentEconomySimulationInvariants(result) {
    ...
    for (const action of requiredActions) {
        if (!actions.has(action))
            throw new Error(`missing receipt action in simulation: ${action}`);
    }
    if (result.steps.length === 0)
        throw new Error('simulation produced no training steps');
    ...
    if (result.summary.world_events_started !== 1 ||
