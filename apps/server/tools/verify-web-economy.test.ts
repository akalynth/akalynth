...
async function main(): Promise<void> {
  ...
  res = await request('POST', '/v1/shop/purchase', { character_id: 'p_buyer' }, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  ...
  res = await request('POST', '/v1/work/start', {}, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  check('work start requires character id', res.status === 400 && res.body.error === 'character_id_required');
  check('missing-character work start emits no receipts', receipts.filter((r) => r.action === WORK_CONTRACT_STARTED_ACTION).length === 0);
  ...
  res = await request('POST', '/v1/work/tick', { contract_id: contractId }, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  ...
  res = await request('POST', '/v1/property/buy', { property_id: 'Azura:H1' }, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  check('property buy requires character id', res.status === 400 && res.body.error === 'character_id_required');
  check('missing-character property buy emits no debit/purchase receipt', receipts.every((r) => r.action !== WALLET_DEBIT_ACTION && r.action !== PROPERTY_PURCHASED_ACTION));

  res = await request('POST', '/v1/property/buy', { character_id: 'p_buyer' }, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  ...
  res = await request('POST', '/v1/property/list', { property_id: 'Azura:H1', price_gold: 75 }, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  ...
  res = await request('POST', '/v1/property/unlist', { character_id: 'p_buyer', property_id: 'Azura:UNKNOWN' }, { cookie: cookieHeader(), 'x-csrf-token': 'csrf-ok' });
  check('property unlist rejects unknown plot', res.status === 404 && res.body.error === 'unknown_plot');
  check('unknown-plot property unlist emits no second unlist receipt', receipts.filter((r) => r.action === PROPERTY_UNLISTED_ACTION).length === 1);
