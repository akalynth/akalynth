...
//
// Examples:
//   npx tsx apps/server/tools/beta-cohort.ts create --cohort beta-2026-07-11 --release <sha> --cap 20 --rollback <sha>
//   npx tsx apps/server/tools/beta-cohort.ts issue --cohort beta-2026-07-11 --count 5
//   npx tsx apps/server/tools/beta-cohort.ts list
//   npx tsx apps/server/tools/beta-cohort.ts triage --feedback bf_<id> --status in_progress --owner ops
//
// Raw invite codes are printed once for operator delivery and are never logged
// to receipts or persisted. Run this from the repository root.
...
function audit() {
  return createAuditLogger({ receiptPath: paths.receiptsPath, keyPath: paths.keyPath ?? undefined });
}
...
try {
  ...
  if (command === 'create') {
    ...
    if (platform !== 'web' && platform !== 'android' && platform !== 'mixed') throw new Error('--platform must be web, android, or mixed');
    ...
    console.log(JSON.stringify({ ok: true, cohort_id: cohortId, release_commit: release, invite_cap: cap, rollback_commit: rollback }, null, 2));
    ...
    try {
      ...
      const issued: Array<{ invite_id: string; invite_code: string; invite_hint: string; cohort_id: string }> = [];
      for (let index = 0; index < count; index += 1) {
        ...
        logger.write({
          ...
          inputs: { invite_id: inviteId, cohort_id: cohortId, token_hint: hint, release_commit: cohort.release_commit },
          result: 'issued',
        });
        issued.push({ invite_id: inviteId, invite_code: code, invite_hint: hint, cohort_id: cohortId });
      }
      console.log(JSON.stringify({ ok: true, cohort_id: cohortId, expires_at: isoAfterDays(expiresDays), invites: issued }, null, 2));
    } finally {
      logger.close();
    }
    ...
    const cohortId = required('cohort');
    const status = command === 'pause' ? 'paused' : command === 'close' ? 'closed' : 'open';
    ...
    if (!['open', 'triaged', 'in_progress', 'fixed', 'closed'].includes(status)) throw new Error('invalid feedback status');
    if (!/^[A-Za-z0-9_.:-]{1,64}$/.test(owner)) throw new Error('invalid owner');
    const logger = audit();
    try {
      logger.write({
        actor_id: 'beta_operator',
    ...
    console.log(JSON.stringify({ ok: true, feedback_id: feedbackId, status, owner }));
