// Focused regression checks for Beta Player Readiness and Measurement v1.
...
try {
  initSchema(db);
  assert.equal(db.prepare("SELECT value FROM _meta WHERE key = 'schema_version'").get()?.value, String(SCHEMA_VERSION));
  ...
  assert.throws(() => store.issueInvite({
  ...
  const stored = db.prepare('SELECT token_hash FROM beta_invites WHERE invite_id = ?').get('verify-invite') as { token_hash: string };
  assert.equal(stored.token_hash, hashToken(rawInvite));
  assert.notEqual(stored.token_hash, rawInvite);

  const receipts: Array<{ action: string; inputs: Record<string, unknown> }> = [];
  const service = new BetaService({
    ...
    now: () => Date.now(),
    emitReceipt: (event) => receipts.push({ action: event.action, inputs: event.inputs }),
  });
  ...
  assert.equal(service.status('acc_verify').cohort?.cohort_id, 'verify-cohort');
  ...
  const badEvent = service.recordEvent(null, { event: 'world_state_reached', client_session_id: 'bad' });
  assert.equal('error' in badEvent && badEvent.error, 'invalid_client_session_id');
  ...
  assert.ok(receipts.some((receipt) => receipt.action === 'beta_invite_redeemed'));
  assert.ok(receipts.some((receipt) => receipt.action === 'beta_event_recorded'));
  assert.ok(receipts.some((receipt) => receipt.action === 'beta_feedback_submitted'));
  ...
  const accountService = new AccountService({
    ...
    emitReceipt: (event) => receipts.push({ action: event.action, inputs: event.inputs ?? {} }),
    now: () => Date.now(),
    config: { secureCookies: false, sessionTtlSec: 3600, verificationTtlSec: 3600, resetTtlSec: 3600, devExposeLinks: false },
    beta: { requireInvite: true, claimInvite: (code, accountId) => service.claimInvite(code, accountId) },
  });
  const missingInvite = await accountService.register({ handle: 'BetaPlayer0', password: 'password123' });
  assert.equal(missingInvite.status, 403);
  const registered = await accountService.register({ handle: 'BetaPlayer', password: 'password123', invite_code: newToken(24) });
