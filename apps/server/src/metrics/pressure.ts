...
import type { PressureMetrics, PressureMetricsContributors } from '../../../../packages/shared/protocol.js';
import type { ChronicleEventRow } from '../persist/types.js';
import {
  getChronicleRange,
  getDeathsRange,
  getPlayerHeatSummary,
  getSchemaVersion,
...
export interface PressureMetricsContext {
  db: Database.Database;
}

export interface PressureMetricsResult {
  status: 'ok' | 'not_ready';
...
export function computePressureMetrics(
  ctx: PressureMetricsContext,
  playerId: string,
  since?: string,
  until?: string
): PressureMetricsResult {
  const { db } = ctx;
  ...
  const items_lost_by_type: Record<string, number> = {};
  for (const event of lostEvents) {
    const details = parseDetails(event.details_json);
    const itemType = (details.item_type as string) ?? 'unknown';
  ...
  let drop_count = 0;
  let worst_death: { receipt_hash: string; drop_ratio: number } | undefined;

  for (const deathEvent of deathEvents) {
    const details = parseDetails(deathEvent.details_json);
    const dropped = details.dropped_count as number | undefined;
    const eligible = details.eligible_count as number | undefined;
  ...
  // -------------------------------------------------------------------------
  const contributors: PressureMetricsContributors = {
    ...
    evidence_receipt_hashes: deathEvents
      ...
      .map((e) => {
        ...
        return ref.receipt_hash as string;
      })
      .filter(Boolean),
  };
 ...
 */
function computeExposureMinutes(
  lostEvents: ChronicleEventRow[],
  windowStart: Date,
  windowEnd: Date
): number {
  let totalMinutes = 0;
