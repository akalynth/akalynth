...
function validateCompensationRef(
  ref: unknown,
  path: string,
  issues: ValidationIssue[],
): boolean {
  if (!isRecord(ref)) {
    addIssue(issues, 'MALFORMED', path, 'must be a compensation reference');
    return false;
  }
  const rootOk = requireString(ref.root_id, `${path}.root_id`, issues);
  const effectOk = requireString(
    ref.logical_effect_id,
    `${path}.logical_effect_id`,
    issues,
  );
  return rootOk && effectOk;
}
...
function addBudgets(budgets: readonly BudgetVector[]): BudgetVector {
  const total = Object.create(null) as Record<string, number>;
  for (const budget of budgets) {
    for (const [dimension, amount] of Object.entries(budget)) {
      total[dimension] = budgetAmount(total, dimension) + amount;
    }
  }
  return total;
}

function budgetAmount(budget: BudgetVector, dimension: string): number {
  return Object.prototype.hasOwnProperty.call(budget, dimension)
    ? (budget[dimension] ?? 0)
    : 0;
}
...
function budgetWithin(actual: BudgetVector, limit: BudgetVector): boolean {
  return Object.entries(actual).every(
    ([dimension, amount]) => amount <= budgetAmount(limit, dimension),
  );
}
...
function budgetsEqual(left: BudgetVector, right: BudgetVector): boolean {
  ...
  for (const dimension of dimensions) {
    if (budgetAmount(left, dimension) !== budgetAmount(right, dimension)) {
      return false;
    }
  }
  return true;
...
export function validateEnvelope(
  ...
  validateOptionalString(
    envelope.client_correlation_key,
    'envelope.client_correlation_key',
    issues,
  );
  ...
  if (envelope.envelope_kind === 'ece') {
    ...
    if (
      ...
      && envelope.protection_commitment !== undefined
    ) {
      if (!isRecord(envelope.protection_commitment)) {
        addIssue(
          issues,
          'MALFORMED',
          'envelope.protection_commitment',
          'must be a protection commitment reference',
        );
      } else {
        requireString(
...
export function validateBasis(
  ...
  if (
    basis.trigger.evidence_ref !== null
    && basis.trigger.evidence_ref !== undefined
  ) {
    validateArtifactRef(
  ...
  if (
    basis.server_time_boundary !== null
    && basis.server_time_boundary !== undefined
  ) {
    validateArtifactRef(
  ...
  if (basis.rng !== null && basis.rng !== undefined) {
    if (!isRecord(basis.rng)) {
      addIssue(
        issues,
        'MALFORMED',
        'basis.rng',
        'must be an RNG resolution basis',
      );
    } else {
      validateArtifactRef(
...
function validateEffect(
  ...
  const compensationRef = effect.compensation_of;
  const compensationRefValid =
    compensationRef !== null
    && compensationRef !== undefined
      ? validateCompensationRef(
          compensationRef,
  ...
  if (envelope.envelope_kind === 'rce' || envelope.envelope_kind === 'mce') {
    ...
    if (compensationRefValid && compensationRef) {
      ...
      const allowedEffectIds =
        envelope.envelope_kind === 'rce'
          ...
          : envelope.target_effect_ids;
      if (
        compensationRef.root_id !== expectedRootId
        || !allowedEffectIds.includes(
          compensationRef.logical_effect_id,
        )
      ) {
        addIssue(
          issues,
