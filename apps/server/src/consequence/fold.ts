...
function accountingAmount(
  ...
  return Object.prototype.hasOwnProperty.call(accounting, dimension)
    ? (accounting[dimension] ?? 0)
    : 0;
}
...
function validateRecoveryParent(
  ...
  if (envelope.reservation.kind === 'scalar') {
    ...
    const alreadyAllocated = Object.create(null) as Record<string, number>;
    for (const current of episodes.values()) {
      ...
      for (const [dimension, amount] of Object.entries(allocation)) {
        alreadyAllocated[dimension] =
          accountingAmount(alreadyAllocated, dimension) + amount;
      }
    }
    for (const [dimension, requested] of Object.entries(
      ...
      const total =
        accountingAmount(
          reservedEffect.recoverable.recoverable_total,
          dimension,
        );
      if (
        accountingAmount(alreadyAllocated, dimension) + requested
        > total
      ) {
        faults.push(
          integrityIssue(
...
function addAccounting(
  ...
  for (const [dimension, amount] of Object.entries(accounting)) {
    target[dimension] = accountingAmount(target, dimension) + amount;
  }
}
...
function validateRemediationParent(
  ...
  const targetAccounting = Object.create(null) as Record<string, number>;
  ...
  for (const [dimension, amount] of Object.entries(
    ...
    if (amount > accountingAmount(targetAccounting, dimension)) {
      faults.push(
        integrityIssue(
          'ACCOUNTING_EXCEEDED',
