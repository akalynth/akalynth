...
export async function callXaiLLM(prompt, { model = XAI_MODEL, temperature = 0.7 } = {}) {
  ...
  // Use Responses API (preferred per current docs.x.ai). Send full decision prompt as input.
  // We rely on strong instruction in the prompt + robust parse for JSON-only response.
  const res = await fetch(`${XAI_BASE}/responses`, {
    ...
    body: JSON.stringify({
      ...
      // temperature may be passed; some models are sensitive — default is fine
      // store: false to avoid unnecessary server history for sim runs
      store: false
    })
  });
  ...
  const data = await res.json();
  // Responses API typically surfaces text via output_text or output[].content[].text
  let text = '';
  if (typeof data.output_text === 'string') {
    ...
    // Fallback if a chat.completions-shaped response is returned
...
export async function proposeDecision(context) {
  ...
  try {
    ...
    // Log provider failure at call site if desired; always degrade gracefully
