...
import type { AntiCheatPriorRecord, AntiCheatPriorResponse } from '../../../../packages/shared/http.js';

type PriorLookupError = { error: string; status: number };

export type AntiCheatPriorLookupResult = AntiCheatPriorResponse | PriorLookupError;

export interface AntiCheatPriorStoreConfig {
  enabled: boolean;
  filePath?: string;
}

export interface AntiCheatPriorStore {
  queryPlayerPrior(playerId: string): AntiCheatPriorLookupResult;
}

export function createAntiCheatPriorStore(config: AntiCheatPriorStoreConfig): AntiCheatPriorStore {
  let cache = new Map<string, AntiCheatPriorRecord>();
  let cacheKey: string | null = null;

  function currentCacheKey(): string {
    if (!config.filePath) return 'missing';
  ...
  function loadIfNeeded() {
    ...
    const nextCache = new Map<string, AntiCheatPriorRecord>();
...
function parsePriorLine(line: string): AntiCheatPriorRecord {
  let raw: unknown;
  try {
    raw = JSON.parse(line);
  } catch {
...
function normalizeTopSignal(value: unknown) {
