...
const SOURCE_MAP_NAME: Record<string, MapName> = {
  rookguard: 'Rookguard',
};

export function buildPreviewWorldFork(manifest: BuilderDraftManifest): BuilderPreviewWorldFork {
  ...
  return {
    ...
    objects: (manifest.objects ?? [])
      .filter((obj): obj is typeof obj & { placement: [number, number] } => !!obj.placement)
      .map((obj) => ({
        id: obj.id,
        kind: obj.kind,
        text: obj.text,
