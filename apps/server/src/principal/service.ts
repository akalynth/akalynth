...
import type {
  PrincipalChallengePayload,
  PrincipalChallengePurpose,
} from './canonical.js';
import {
  PRINCIPAL_CHALLENGE_TYPE,
  PRINCIPAL_PROTOCOL_VERSION,
  canonicalJson,
  isPrincipalChallengePurpose,
  publicKeyFingerprint,
  sha256Hex,
  verifyDeviceSignature,
} from './canonical.js';
import type {
  PrincipalModerationResolution,
  PrincipalReportRow,
  PrincipalRow,
  PrincipalSessionRow,
} from './store.js';
import { PrincipalStore } from './store.js';
...
export interface PrincipalServiceConfig {
  domain: string;
...
export interface PrincipalServiceDeps {
  ...
  emitReceipt: (e: { action: string; principalId: string | null; inputs?: Record<string, unknown>; result: string }) => void;
  now: () => number;
  config: PrincipalServiceConfig;
}
...
function normalizeHandle(handle: string): { handle: string; lower: string } | null {
  ...
  if (reserved.has(lower) || lower.startsWith('guest_') || lower.startsWith('deleted_')) return null;
  return { handle: trimmed, lower };
}

function parseRoles(row: PrincipalRow): string[] {
  try {
...
export function deriveCapabilities(roles: string[], identityLevel: PrincipalIdentityLevel): PrincipalCapability[] {
  const caps = new Set<PrincipalCapability>();
  caps.add('forum:post_basic');
  caps.add('forum:report');
  caps.add('forum:block');

  if (identityLevel === 'pgp_bound') caps.add('forum:post_authority');
...
function publicPrincipal(row: PrincipalRow, identityLevel: PrincipalIdentityLevel = 'key_bound') {
...
function cleanReason(value: unknown): string | null {
  ...
  if (!trimmed || trimmed.length > MAX_REASON_LEN) return null;
  return trimmed;
}
...
export class PrincipalService {
  constructor(private readonly d: PrincipalServiceDeps) {}

  register(input: Record<string, unknown>): PrincipalResponse {
  ...
  challenge(input: Record<string, unknown>): PrincipalResponse {
    ...
    const payload: PrincipalChallengePayload = {
  ...
  verify(input: Record<string, unknown>): PrincipalResponse {
    ...
    const identityLevel: PrincipalIdentityLevel = 'key_bound';
  ...
  me(authHeader: string | undefined): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
    return { status: 200, body: { ok: true, principal: publicPrincipal(ctx.principal, ctx.session.identity_level as PrincipalIdentityLevel) } };
  }

  acceptTerms(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
  ...
  block(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
    if (!ctx.capabilities.includes('forum:block')) return { status: 403, body: { ok: false, error: 'capability_required' } };
  ...
  report(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
    if (!ctx.capabilities.includes('forum:report')) return { status: 403, body: { ok: false, error: 'capability_required' } };
    const target = typeof input.target_principal_id === 'string' ? input.target_principal_id : '';
    const reason = cleanReason(input.reason);
  ...
  listReports(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
    if (!ctx.capabilities.includes('moderation:read')) return { status: 403, body: { ok: false, error: 'capability_required' } };
  ...
  resolveReport(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
    if (!ctx.capabilities.includes('moderation:resolve')) return { status: 403, body: { ok: false, error: 'capability_required' } };
    ...
    if (!report) return { status: 404, body: { ok: false, error: 'not_found' } };
    if (report.status === 'resolved') return { status: 409, body: { ok: false, error: 'already_resolved' } };
  ...
  retire(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
  ...
  deletePrincipal(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
  ...
  pgpBind(authHeader: string | undefined, input: Record<string, unknown>): PrincipalResponse {
    const ctx = this.resolveAuth(authHeader);
    if (!ctx) return { status: 401, body: { ok: false, error: 'not_authenticated' } };
  ...
  resolveAuth(authHeader: string | undefined): PrincipalAuthContext | null {
    const token = parseBearer(authHeader);
    if (!token) return null;
    const session = this.d.store.findSessionByTokenHash(hashToken(token));
    if (!session || session.revoked_at || Date.parse(session.expires_at) <= this.d.now()) return null;
    ...
    const identityLevel = session.identity_level as PrincipalIdentityLevel;
  ...
  private verifyChallenge(
    ...
    expectedPurpose: PrincipalChallengePurpose,
  ):
    | { ok: true; principal: PrincipalRow; keyFingerprint: string }
    | { ok: false; response: PrincipalResponse } {
    const principalId = typeof input.principal_id === 'string' ? input.principal_id : '';
...
function isResolution(value: unknown): value is PrincipalModerationResolution {
...
function isStoredPayload(
  ...
  expectedPurpose: PrincipalChallengePurpose,
  domain: string,
): payload is PrincipalChallengePayload {
  if (!isRecord(payload)) return false;
  return (
    payload.type === PRINCIPAL_CHALLENGE_TYPE &&
