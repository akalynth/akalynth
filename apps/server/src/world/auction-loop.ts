...
import type { PropertyAuctionKind, WalletCreditReason } from '../../../../packages/shared/types.js';
...
export function clampAuctionDurationS(durationS: unknown): number {
  const n =
...
export interface AuctionSettlement {
  ...
  winner_id: string | null; // null = no bids
  seller_id: string | null; // null = primary (treasury)
  price: number;
  sale_count: number;
}
 ...
 */
export function settleDueAuctions(nowMs: number, writeReceipt: WriteReceiptFn): AuctionSettlement[] {
  const settled: AuctionSettlement[] = [];

  for (const auction of getOpenAuctions()) {
    if (auction.scheduled_close_ms == null) continue; // no recorded close
    if (nowMs < auction.scheduled_close_ms) continue; // not yet due
    ...
    if (winnerId && auction.kind === 'resale' && auction.seller_id) {
      writeReceipt({
        ...
        inputs: { amount: price, reason: `auction_sale:${auction.property_id}` as WalletCreditReason },
        result: 'ok',
      });
    }
