...
export interface BetaServiceDeps {
  ...
  now: () => number;
  emitReceipt: (e: { actorId: string; action: string; inputs: Record<string, unknown>; result: string }) => void;
}
...
export class BetaService {
  ...
  claimInvite(code: unknown, accountId: string): BetaInviteClaim | BetaInviteClaimFailure {
    const raw = typeof code === 'string' ? code.trim() : '';
    if (!raw) {
      return this.d.requireInvite
        ? { ok: false, status: 403, error: 'beta_invite_required' }
        : { ok: true, cohort: null };
    }
    if (this.d.store.cohortForAccount(accountId)) {
      return { ok: false, status: 409, error: 'beta_invite_unavailable' };
    }
    const claimed = this.d.store.claimInvite(hashToken(raw), accountId, iso(this.d.now()));
    if (!claimed.ok) {
      return { ok: false, status: claimed.reason === 'cohort_closed' ? 409 : 403, error: claimed.reason === 'cohort_closed' ? 'beta_invite_unavailable' : 'beta_invite_invalid' };
    }
    const cohort = {
  ...
  recordEvent(accountId: string | null, input: BetaReadinessEventRequest): BetaReadinessEventResponse | { status: 400; error: string } {
    ...
    const reason = input.reason === undefined ? null : typeof input.reason === 'string' && SAFE_REASON_RE.test(input.reason) ? input.reason : null;
    const durationMs = input.duration_ms === undefined ? null : typeof input.duration_ms === 'number' && Number.isInteger(input.duration_ms) && input.duration_ms >= 0 && input.duration_ms <= 86_400_000 ? input.duration_ms : null;
  ...
  submitFeedback(accountId: string, input: BetaFeedbackRequest): BetaFeedbackResponse | { status: 400; error: string } {
    ...
    if (input.client_session_id !== undefined && !clientSessionId) return { status: 400, error: 'invalid_client_session_id' };
