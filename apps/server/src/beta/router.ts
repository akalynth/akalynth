...
export function makeBetaRouter(deps: BetaRouterDeps) {
  return async function handleBeta(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
    ...
    if (path === '/v1/beta/me' && method === 'GET') {
      const a = account();
      if (!a) return (send(res, 401, { ok: false, error: 'not_authenticated' }), true);
      send(res, 200, deps.service.status(a.accountId));
      return true;
    }

    if (path === '/v1/beta/events' && method === 'POST') {
      const rl = deps.eventLimiter.check(`beta-event:${clientIp(req)}`);
      if (!rl.ok) return (send(res, 429, { ok: false, error: 'rate_limited', retry_after_sec: rl.retryAfterSec }), true);
      const result = deps.service.recordEvent(account()?.accountId ?? null, await readJson(req) as unknown as BetaReadinessEventRequest);
    ...
    if (path === '/v1/beta/feedback' && method === 'POST') {
      const rl = deps.feedbackLimiter.check(`beta-feedback:${clientIp(req)}`);
      if (!rl.ok) return (send(res, 429, { ok: false, error: 'rate_limited', retry_after_sec: rl.retryAfterSec }), true);
      const a = account();
      if (!a) return (send(res, 401, { ok: false, error: 'not_authenticated' }), true);
      ...
      const result = deps.service.submitFeedback(a.accountId, await readJson(req) as unknown as BetaFeedbackRequest);
    ...
    if (path === '/v1/beta/invites/redeem' && method === 'POST') {
      ...
      const result = deps.service.claimInvite(body.invite_code, a.accountId);
      if (!result.ok) {
