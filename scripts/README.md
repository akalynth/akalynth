...
- Root `npm run verify:account-character`: focused account-character parity
  gate. Runs protocol sync, server account-character tests, server create/select play-token handoff and login projection proof, server wallet/shop/work/property gameplay route proof, debug-client guard, Android account-character unit tests, Android gameplay wire-authority protocol proof, and Android character UI compile.
