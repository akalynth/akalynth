...
| `AKALYNTH_SYSTEMS_BIBLE_V1/` + `.zip` | Systems source: progression, combat, economy, crafting, reputation, world state, failure states, social orgs, first playable slice. | Indexed by `docs/asset-decisions/AKALYNTH_SYSTEMS_BIBLE_SOURCE_INTAKE_V1/SYSTEMS_BIBLE_SOURCE_INTAKE.md`. Keep local. Any implementation implies explicit protocol/persistence/economy/receipt/anti-cheat review. |
...
| `AKALYNTH_CINDERWATCH_FRONTIER_SLICE_V1/` + `.zip` | Frontier/survival source: Cinderwatch Camp, patrol contracts, creature tracking, caravan/refugee escort, camp condition, fortification, Glassfang Brood, Glassfang Matriarch, frontier policy choice, and Frontier Ashfall seed. | Keep local. Indexed by `docs/asset-decisions/AKALYNTH_CINDERWATCH_FRONTIER_SOURCE_INTAKE_V1/CINDERWATCH_FRONTIER_SOURCE_INTAKE.md`. No runtime promotion, no raw import, and no live frontier travel, patrol board, tracking state, escort system, camp state, boss, reward, faction consequence, Chronicle output, or world-event schedule. |
| `AKALYNTH_WORLD_EVENTS_ENGINE_V1/` + `.zip` | World-events source: event lifecycle, event types, canon cards, contribution scoring, rewards, failure/aftermath, prototype event, integration matrix, release gates. | Indexed by `docs/asset-decisions/AKALYNTH_WORLD_EVENTS_ENGINE_SOURCE_INTAKE_V1/WORLD_EVENTS_ENGINE_SOURCE_INTAKE.md`. Keep local. The reviewed runtime seed is the Witness Moth Bloom prototype only; no raw package imports and no client-authoritative event state. |
| `AKALYNTH_CAMPAIGN_ACT_I_V1/` | Act I campaign spine source: "The Four Proofs" arc connecting the High City, Ember Road, Moonspire, and Cinderwatch slices; prologue, four chapters, proof types, origin paths, per-chapter major choices, finale Convergence responses, Chronicle records, and release gates. | Keep local. Indexed by `docs/asset-decisions/AKALYNTH_CAMPAIGN_ACT_I_SOURCE_INTAKE_V1/CAMPAIGN_ACT_I_SOURCE_INTAKE.md`. Connective design over the four existing slices; reuses their vocabulary and does not fork their internal designs. No runtime promotion, no raw import, and no live campaign state, chapter gating, proof tracking, finale council, choice persistence, or Chronicle output. |
