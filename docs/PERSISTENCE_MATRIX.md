...
| Controlled-beta cohort and invite ledger | Server/operator beta service | `D2_sqlite_local` (`beta_cohorts`, `beta_invites`, schema v27) operational state | hashed invite rows plus canonical release/rollback manifest digests; beta invite issue/redeem receipts where emitted | Controlled-cohort operations only; no gameplay, world-truth, retention, or release claim | Unbound legacy rows are admission-inert; receipt every cohort lifecycle mutation or define backup/restore proof before claiming full reconstructibility. |
