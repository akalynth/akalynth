...
| Witness requests/cooldowns | Server witness runtime | `D0_in_memory` (intentionally ephemeral) | witness_requested / witness_response receipts where emitted | Restart-reset acceptable: 12s request TTL + 60s cooldowns are shorter than any restart; durable consequence is the quorum outcome (below) and heat (above) | None required for MVP; document as intentional. |
...
| Rookguard quest progress | Server quest projection | `D3_ci_fixture_proved` for tutorial/training/vocation/gate projection replay from receipts | tutorial_step_complete, mob_kill, vocation_declared, gate_unlock, tutorial_completed receipts; `verify:rookguard-quest` | Local verifier-backed reconnect/replay behavior for the named Rookguard Codex path only | Add focused WebSocket reconnect scenario and production restart proof before release-claiming durability. |
| Rookguard canal fishing state | Server world projection | `D1_file_receipted` plus a replayed in-memory projection and local verifier | `rookguard_canal_fished` and `rookguard_canal_merchant_reacted` receipts; derived Chronicle `world_event` rows; `verify:rookguard-fishing` | Local source-level recovery/reconnect/replay proof only; no economy or production durability claim | Wire the verifier into CI, then capture a production restart proof before raising the durability class. |
