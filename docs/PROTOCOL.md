...
`packages/shared/protocol.ts` exports `PROTOCOL_VERSION = '2.2.0'`. v2.2.0
is additive over v2.1.0 and records the gather, refine, and delivery intent/result
families; gather node/station shared types and rejection aliases; optional
`InventorySnapshotMessage.houseStorage`; and optional
`WorldStateMessage.builder_preview`. Existing 2.x clients remain
major-version compatible and must ignore unknown optional fields and message
types. v2.1.0 is additive over v2.0.0 and records `loop_update` as a first-class
server progression message. v2.0.0 accepts the house-auction protocol surface:
auction open/bid/cancel client intents, auction state and settlement server
broadcasts, and widened `property_result` action/reason values. Clients with
exhaustive `property_result.action` or `property_result.reason` handling must
tolerate the auction values listed here.
...
`route:craft:ashglass`, `route:craft:refine`, `route:craft:mint`,
`route:economy:settle`, `route:economy:payout`,
`route:survey:moonspire`, `route:dream:interpret`,
`route:dream:fragment`, `route:safety:moonspire`,
`route:gate:moonspire`, `route:dream:traverse`, and
`route:dream:arrive`.
The server validates sequencing from receipts and returns `invalid_target`
without route side-effect receipts when a route step is locked, out of order,
