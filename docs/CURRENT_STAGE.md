...
- Rookguard Fish causal-world proof: the existing Android/browser `activity:fishing:rookguard` intent now resolves through receipt-backed canal recovery and a non-economic merchant reaction, projects both stages into the existing Chronicle `world_event` shape, exposes replayed state through `PlayLoopProgress.fishing`, and derives elapsed recovery from a receipt-authored deadline. Covered by `apps/server` `npm run verify:rookguard-fishing` plus Android protocol/action tests. This is a local source-level proof, with no new action UI, item mint, price change, broader agent simulation, or production proof run claim.
- Agent economy simulator proof: deterministic offline worker, homesteader, and merchant roles traverse Rookguard/Azura, emit simulator receipts for economy/gameplay systems, and materialize those receipts through SQLite projection checks. Covered by `apps/server` `npm run verify:agent-economy-simulation` and included in `npm run verify:showcase`. This is a local proof-packet input, not a production, launch, Android release, or content-alpha claim.
